...
from .features import DinoExtractor, array_features, file_key
...
def process_video(frames: np.ndarray, key: str, cache_dir: pathlib.Path,
...
def process_video_file(path: pathlib.Path, cache_dir: pathlib.Path,
                       extractor: DinoExtractor, tracker: Tracker | None = None,
                       short_side: int = 256, **morph_kw) -> tuple[VideoBundle, np.ndarray]:
