...
**Version: `transition-eval/3.0.0-draft.8`** (see `VERSION`; stamped by `versioning.py`)
...
| O7  | ~~M1b robust weighting choice~~ **TRIGGERED at the draft.7 cert** (camera-stratum mean recall 0.346 < 0.5) → per the pre-registration, draft.8 implements Huber IRLS (`_fit_similarity_huber`) and examines it vs median-trim under the same adoption rule (`adopt_camera_fit`); the winner deploys via `score.py --camera-fit`                                                                                                                                                                                                | ~~cert~~ done | §6.1 / `bars.yaml`                                                                          |
...
**M1b — Camera motion match.** Per-step robust visibility-weighted similarity fit `(dx, dy, log s, θ)` on tracklets (median-trim or Huber IRLS — the O7 adoption in §6.1 decides which is deployed, `score.py --camera-fit`) → 4-channel trajectory, resample 64, z-norm → banded DTW (band 0.15) + correlation vs reference. ↑corr/↓DTW. *Blind to:* parallax/3D (SfM rejected: scenes are non-rigid/dissolving); flagged when trackable points < N.
...
**Variants (only these):** core mask {v2_envelope, v3_sided, all_frames}; motion {v2 MFS incumbent, v3 decomposed}; camera fit {median-trim, Huber IRLS}. **Adoption (pre-registered, `exam.py` applies it mechanically):** one-sided exact binomial sign test (α=0.05) on per-class recall over the non-tied n≥4 classes of the target stratum ∧ overall accuracy within tolerance of the incumbent ∧ no trusted class regresses below the trust floor ∧ (core mask only) non-degenerate STRICT cores on ≥ bar fraction of real one-sided clips. **Motion contingency (pre-registered):** decomposition additionally requires the M1c median recall on definedness-eligible classes to trail the incumbent by ≤ bar; if not adopted, the incumbent MFS ships as the single headline motion metric and M1b/M1c demote to analysis tier. **O7 conditional — TRIGGERED by the draft.7 exam** (camera-stratum mean recall 0.346 < trust floor): per the pre-registration, this draft examines Huber IRLS vs the median-trim primary under the *same* adoption rule (`adopt_camera_fit`: camera-stratum sign test + M1c guard); the winner is the deployed fit, threaded to `score.py` as `--camera-fit` so every downstream stage scores with the adopted statistic.

**Bar 1 (draft.8 form — draft.7's single global floor failed and its diagnosis is recorded in `certifications/v3.0.0-draft.7.md`):** the teeth are a **precedent-reproduction clause**: LOO accuracy ≥ 0.80 ∧ d ≥ 1.5 computed on the exam restricted to the 11 exp_054 precedent styles — the gallery where 0.851/d 2.22 was actually measured (all two-sided, so the adopted sided mask coincides with the precedent's envelope there; singletons count as misses exactly as in the precedent). Global clauses acc ≥ 0.60 ∧ d ≥ 1.5 are **regression guards knowingly anchored under the draft.7 measured values** (0.673/1.522) — precedent-anchoring on *this* corpus, protecting future re-runs. Global pooled accuracy beyond the guard is a descriptive headline: at 39 fine-grained classes it conflates label granularity with metric quality; per-class claims live in the trust map.
...
- **Siblings.** All within-class pairs of n≥2 classes feed the **content-invariance audit** via cached-feature statistics (deployed set-similarity + endpoint vectors — no full scoring; ~500 pairs would waste hours of LPIPS for numbers the audit doesn't need). Hard bars attach only to the **max-endpoint-distance pair per class** — deterministic from cached features; same style, maximally different content, the constructional known-positive — which runs through full `score.py`. (Lexicographic pairing rejected: filename adjacency correlates with shared source scenes, inflating M1a and legitimately firing M2a.) **Bar 2:** on those pairs, M1a > class control ∧ M2a silent, ≥ bar count of the 37 eligible classes.
- **Controls.** lerp (two-sided) / static-hold (one-sided) — the sibling items' auto-synthesized control arms. **Bar 3 (draft.8 form):** control M1a below its sibling (the floor claim, same units) for EVERY control; static-hold controls must **additionally** fire `core_degenerate` (draft.7 validated this 26/26); lerp controls carry no degeneracy claim — a pixel blend sits far from *both* endpoints in DINO space and legitimately earns strict-core frames (draft.7 measured 8–29 on 10/11 lerps; the conjunction encoded a false hypothesis and is the recorded reason for draft.7's bar-3 FAIL). Lerp flags are recorded descriptively. ≥ bar count of the 37 sibling-eligible classes; misses documented — documentation never rescues a miss.
...
- **3.0.0-draft.8** (2026-07-13): revisions from the **draft.7 FAILED certification** (job 9463686; record `certifications/v3.0.0-draft.7.md` — bars 2 and 5 passed, 1/3/4/6/7/8 failed; fail-forward, nothing about draft.7 was rescued). (1) **Bar 1 re-derived**: draft.7's global floor 0.80 was anchored to exp_054's 0.851 measured on 47 clips / 11 styles (chance .213) and mechanically failed against 223/39 (chance .067) — new form = precedent-reproduction clause (acc ≥ 0.80 ∧ d ≥ 1.5 on the exam restricted to the 11 precedent styles) + global regression guards (acc ≥ 0.60, d ≥ 1.5) knowingly anchored under the draft.7 measured 0.673/1.522; global pooled accuracy demoted to descriptive. (2) **Bar 3 arm-split**: the `core_degenerate` conjunction was refuted for lerp controls (DINO places pixel blends far from both endpoints — 8–29 legitimate strict-core frames on 10/11 lerps) while holds fired 26/26 → degeneracy clause now gates the hold arm only; floor clause gates every control; min 35/37. (3) **Scorer crash fixed**: `object_match` returned through `_smooth_tracks` with an empty tracklet keep-filter on low-texture video (splice loops, archived gens) and killed the cert_probes + cert_blockc stages wholesale — now NaN (the documented "no residually-moving tracklets" path), plus a zero-tracklet guard in `camera_trajectory`. (4) **Anchor rule dedup**: strata filled in order, first eligible class *not already picked* (air_bending led two strata; the 6-anchor assertion refused and the driver died pre-record — record was assembled post-hoc from the run's artifacts by the unmodified frozen graders, see `post_hoc_assembly`). Driver now wraps the anchor/cold stage so the record always writes. (5) **O7 executed**: the draft.7 exam triggered the pre-registered conditional (camera stratum 0.346) → Huber IRLS implemented and examined vs median-trim under the same adoption rule; winner deploys end-to-end via `score.py --camera-fit`. Draft.7 descriptive findings carried: content-invariance pooled partial corr **0.82** (≫ 0.4 alarm, non-gating — M1a within-class variation substantially tracks content overlap; nature_bloom = worst case), warm rerun max |Δ| = 0.0 over 74 rows, reversal bar genuine PASS (n=15, p=0.0176).
