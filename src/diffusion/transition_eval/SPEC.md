...
- **Controls.** lerp (two-sided) / static-hold (one-sided) — the sibling items' auto-synthesized control arms. **Bar 3 (draft.8):** control M1a below its sibling (the floor claim, same units), ≥ bar count of 39; misses documented — documentation never rescues a miss. The draft.7 `core_degenerate` conjunct was removed by owner decision: vacuous as a truth claim (trivially true on holds by construction; legitimately false on lerps — DINO places pixel blends far from both endpoints, earning a real strict core, measured 8–29 frames on 10/11 lerps). The flag stays on every row and in the mask-adoption criterion; here it is descriptive.
