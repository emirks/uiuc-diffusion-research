...
**Version: `transition-eval/4.0.0-draft.2`** (see `VERSION`; stamped by `versioning.py`)
**Status: DRAFT** — 4.0.0 replaces the three M1 transfer metrics with the health-validated corpus-relative deliverables (M1a=S3, M1b=D_ZPR, M1c=CSLS) and adds bar 9 (the two-arm causal-excess gate); §4's raw-only invariant is amended to "no outcome-coupled normalization" (corpus-relative reference populations are pinned instrument constants). **draft.2** corrects one pre-registration defect surfaced by the draft.1 run: bar 8's reference-rebuild-parity clause is graded in two classes (value-space arrays keep the scalar `value_tol`; the two ECDF-composed rank-lattice populations `pop_App`/`pop_Dyn` are compared in integer rank units — see §7), because a scalar float tolerance below their 1/(2N) quantum is unsatisfiable under any cross-environment rebuild. Per the 3.0.0 precedent this pure grading-rule change is certified by committed regrade (§6 fail-forward EXCEPTION), not a re-run. A checkout is certified only when `versioning.stamp()` says so (clean tree ∧ exact tag ∧ non-draft ∧ no OPEN pins); 4.0.0 tags as `eval/v4.0.0` on PASS. The prior `eval/v3.0.0` tag stays certified for v3 numbers.
...
- **Fail-forward.** Failed certification → diagnose → new draft version → re-freeze → full re-run. Bars and thresholds are never adjusted after seeing an outcome. **EXCEPTION (3.0.0 + 4.0.0-draft.2 precedent):** when the draft-to-draft delta is a *pure grading-rule change* over data produced under **unchanged measurement pins** — no measurement/grader `.py` touched, reference artifact sha unchanged — the changed clause may be re-graded by a **committed regrade script** with full outcome-aware disclosure, and the byte-identical graders' verdicts carry over verbatim (a re-run only reproduces the same numbers; per-cert determinism is proven — draft.1 warm rerun worst=0.0). The regrade script must be committed in the re-freeze commit *before* it is executed; the record must carry the git diff proving no measurement code changed. Any measurement- or grader-code change still requires a full re-run.
...
- **The reference artifact is an instrument constant** (§4): committed as `src/diffusion/transition_eval/reference_v4.npz`, built by `scripts/build_reference_v4.py` from the pinned corpus, stamped by corpus hash + its own sha256. Certification asserts **rebuild parity** — rebuilding it from the pinned corpus reproduces the committed artifact (gated inside bar 8). **Two-class criterion (4.0.0-draft.2).** Seven of the nine ECDF populations are value-space (raw distances) and must rebuild within `reference.value_tol` (1e-6). The remaining two — `pop_App`, `pop_Dyn` — are **ECDF-composed rank lattices**: `build_reference_from_parts` stores `population(ecdf_fuse([·,·]))`, so every entry sits on the grid {k/(2N)} (N = 24753 corpus pairs), quantum 1/(2N) = 2.0200e-05. A scalar float tolerance *below* that quantum is mechanically unsatisfiable under any cross-environment rebuild — the raw channels feeding the composition (P2 endpoint-anchor mean, Z/P float32 views) reduce in float32, whose order varies by CPU/BLAS, so a ~2.5e-8 raw drift flips one composed cell by exactly one lattice step (draft.1: `pop_App` at 2.02e-05 = 1/(2N), one step; the same clause already tolerated the 2.5e-8 cause). The lattice arrays are therefore compared in **integer rank units** u = round(pop·2N): equal length ∧ lattice-sanity max|pop·2N − u| < `lattice_sanity_tol` on both fresh and committed ∧ max|Δu| ≤ `max_step` (K=4) ∧ count(u differ) ≤ `max_flips` (B=50, ≈0.2%·N). This keeps full detection power — an off-by-one ranking bug displaces all N entries by ≥2 steps (trips B by ~500×), a wrong corpus/code by thousands — while tolerating benign cross-environment quantization (draft.1/job-9538092: `pop_App` 4 flips/step 1, `pop_Dyn` 0). The `reference.rebuild_tol` key is **retained** for the deployed scalar `compare_reference` path; unifying the deployed comparator with the two-class criterion under a fresh run is a v5 O-item. `workbench/` is **outside** the instrument dirty-scope (committed search history, not the measuring device; git pathspec-excluded in `versioning.INSTRUMENT_PATHS`).
...
- `python -m diffusion.transition_eval.certify.run_certification --corpus C --main-root R` — the §6.5 certification driver (refuses unfrozen bars; GPU node, ~35–40 min warm + the bar-9 datasheet ~30–60 min CPU; writes record + figures + explorer under `outputs/eval/certification/<version>/`). One-time regrade tooling: `scripts/regrade_draft8_to_v3.py` (3.0.0 record provenance); `scripts/regrade_draft1_to_v4.py` (4.0.0 record — regrades bar 8's rebuild-parity clause under the two-class criterion over the draft.1 run artifacts) with `scripts/provenance_rebuild_parity_v4.py` (reproduces the Class-B flip-counts: on-node self-repro + code-path bit-identity + committed-artifact cross-environment provenance).
...
- **4.0.0-draft.2** (2026-07-16): REGRADE certification correcting one pre-registration defect surfaced by the draft.1 run (job 9531327, complete A–D, zero grader crashes). draft.1 FAILED on **bar 8 alone**, and within it only the reference-rebuild-parity sub-check of `pop_App` (max|Δ| 2.02e-05 > the frozen scalar 1e-6); bars 1,2,4,5,6,7,9 all PASSED and bar 8's warm (bit-perfect, worst=0.0) + cold-anchor clauses PASSED. Advised campaign, fail-branch consult (fable-advisor at xhigh): diagnosis **(b) pre-registration defect**, not an instrument failure — `pop_App`/`pop_Dyn` are ECDF-composed rank lattices on {k/(2N)} (quantum 1/(2N)=2.02e-05), for which a scalar tolerance is unsatisfiable under any cross-environment rebuild while the same clause tolerates the ~2.5e-8 raw-channel float32-reduction drift that flips one lattice cell one step. **Correction:** the two-class rebuild-parity criterion (§7; value-space arrays at `value_tol` 1e-6 unchanged, lattice arrays in integer rank units K=4/B=50). **NOTHING ELSE CHANGES** — no measurement/grader `.py`, `reference_v4.npz` byte-identical (sha unchanged). Per the 3.0.0 precedent this pure grading-rule change is certified by **committed regrade** (`scripts/regrade_draft1_to_v4.py`) over the draft.1 artifacts + provenance flip-counts (`scripts/provenance_rebuild_parity_v4.py`, job 9538092: on-node self-repro bit-identical, cert-path vs build-script-path bit-identical, `pop_App` 4 flips/step 1, `pop_Dyn` 0; the committed artifact was built on the Jupyter-pod CPU — a different machine — confirming cross-environment drift, not within-node nondeterminism), **not a re-run** (draft.1 determinism bit-perfect). K/B are outcome-aware, disclosed, grounded on fault-signature separation (~4×/12.5× margins). §6 fail-forward amended to codify the regrade exception. On PASS: drop the draft suffix, record `certifications/v4.0.0.md`, annotated tag `eval/v4.0.0`.
