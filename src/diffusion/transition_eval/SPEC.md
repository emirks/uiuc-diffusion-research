...
**Version: `transition-eval/3.0.0-draft.7`** (see `VERSION`; stamped by `versioning.py`)
...
- **Controls.** lerp (two-sided) / static-hold (one-sided) — the sibling items' auto-synthesized control arms. **Bar 3:** control M1a below its sibling (the floor claim, same units) ∧ `core_degenerate` fires, ≥ bar count of 39; misses documented — documentation never rescues a miss.
