## 2026-08-20
**01:15 — counterfactual_training: 014_dualforce_twin KILLED on eval (advisor R3), primary hypothesis closed.** Eval v4 (evals/016, arm@1000 vs control 012@1000, same instrument/corpus/cache): the counterfactual-twin treatment FAILED 4 pre-registered kill bars — ref-dep gap +15.2pp (control +21.4; narrowed the WRONG way: matched G-fit −7.0pp, mismatched G-ref-control −0.8pp), pooled-same 80.3% (89.6), core_degenerate 21/304 (8), G-zs-same 78.3% (<92.7 floor); near_copy clean 0/304. Notable: the frozen α-probe ROSE 3-4× on the trained checkpoint (α(0.85) 0.016→0.051 @step250, health gate passed) yet every sampled same-typed cell fell — forward redirect-read up, on-path sampled quality down. 3rd additive-objective-surgery death on frozen-base IC-LoRA (007 contrastive / 013 KD / 014 twin). Advisor: record KILL, do NOT run @2000 or tune weights, proceed to RES (shot 2) with a quality-preservation constraint + a SAMPLED (not forward) health gate. Record-closing measurements (α@{500,750,1000} + P3a compliance re-pass) running. F-block proposed to owner (forward-read↛sampled-conversion negative).

## 2026-08-19
**23:58 — counterfactual_training: trained + registered `runs/014_dualforce_twin` (step 1000 shipped).** The counterfactual-twin treatment (advised campaign): per-step pair an S2 row with a same-endpoint byte-exact counterfactual, adding a redirect term (x̂₀-space, σ∈[0.5,0.9]) + differential term (v-space, all t), both middle-masked, on top of plain FM — λ_red 0.5 / λ_diff 0.25, warmup 250, S2a+S2b only. Warm-started runs/002@10000, same shell as control runs/012 (only the objective differs). DeltaAI GH200 (job 2982505), trainer `src/LTX-2-dualforce @ 6dc2f68` (CLEAN tree — reproducible), ckpt sha 20322d10. Implemented `TwinConfig` + `_twin_training_step`. Precheck (advised R1/R2): frozen α probe → proceed-flagged, band [0.5,0.9], S1 dropped (endpoints not pixel-identical); 014 α-trajectory clears the step-500 gate at step 250 (α(0.85) 0.0164→0.0511). Arm `dualforce_twin_neutral` registered + 152-row neutral registry stamped (prompt_sha 0d708175 = 012's family); generating gens/019_dualforce_twin (job 2983886). Eval v4 vs 012@1000 next.
