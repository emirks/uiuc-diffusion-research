## 2026-09-03
- `16:50` **Temporal-dynamics probe — grid Look_u by cell group on the owner's arm list (neutral + effect).** seen / unseen / zero-shot / all capped pooled-% for ic_gen, ctt_v2 (leaky effect), ctt_v3, dualforce_control, dualforce_dcg w1/1.5/3/6, refVFX A/B, VAP (neutral/effect/authorcfg), VFXMaster (same), with the deployed S3 beside it; DCG arms scored from the warm cache via their original sweep paths (no extraction), 291 gens re-extracted for the ic_gen/ctt_v2/ctt_v3 effect arms (jobs 3078713/3078714 on ghx4, DCG group serially on the login node after 206 higher-priority jobs blocked 3078715). Files: misc/2026-09-02_temporal_dynamics_metric/{score_cells.py, SCORE_CELLS_RESULTS.md, REPORT.md § grid Look_u by cell group}. Nothing deployed.
- `13:50` **Temporal-dynamics probe — pillar-v2 exam + 20 arms: no core, size robustness (jobs 3077811/3077812, CPU).** Owner requirements: no core mask; invariance to video size (both-A / both-B / A-vs-B). Measured: all arms are 5.04 s at 480×640 and differ only in frame rate (121/49/33), so the corpus decimated in feature space to 49/33 frames is the exact size test. Pre-registered PREREG_v2 (frame vs grid families, pillars T/Tshape/E/Mu/Mw/D, composite v2u), judged on the frozen exam + size tests + per-class map + ANOVA + hold/cut degeneracy, then all 20 arms. Deployed S3 is not size-robust (33-frame pairs rank 0.36 units more dissimilar; acc 0.807→0.744); the grid Look half (Mu+D, no core, K=32) is (bias 0.50–0.55, acc flat 0.80–0.82) and matches S3 at 121 (0.798 vs 0.807, inside SE). Timing curves are near chance on class retrieval (T 0.16, E 0.25) and the pre-declared v2u composite loses 40 clips — timing goes to validity flags, not the kernel. On arms the size fix moves externals +5 to +16 points (refVFX B 33→49, VFXMaster neutral 48→57) while 121-frame arms move ≤1; externals still trail by 25–35. Files: misc/2026-09-02_temporal_dynamics_metric/{PREREG_v2.md, pillars.py, pillars_exam.py, pillars_arms.py, PILLARS_EXAM_RESULTS.md, PILLARS_ARMS_RESULTS.md, REPORT.md}. Nothing deployed.
- `05:34` **Viewer: added the dualforce_control DCG + base-effect arms.** iclora_neutral_effect now carries the base dualforce_control_effect pill + a 'DCG on dualforce_control' category (8 w-arms x N/E) — 64 toggleable arms (was 55). Builds green (139 cards, 15,262 videos).
- `04:41` **Completed the base dual-force control EFFECT arm.** Generated dualforce_control_effect (runs/012@1000, seeds 42/43, 304 clips) + eval-v4: pooled-same **93.6%** (neutral base 89.6 → effect text lifts appearance). Registered gens/013_dualforce_control/02_effect + evals/026. Fixed run_eval.py report() to honor `no_twin` (a standalone effect baseline has no base twin — mirrors plan()). Base arm now has neutral+effect.
- 04:45 **dualforce `null_contrast` — CERTIFIED co-scored A/B (evals/026).** Folded control+lose (`control_null`, lerp-null reference; runs/020@1000) vs plain-FM control 012, all 4 arms (neutral+effect) in **ONE v4 pass**, seed 42, DeltaAI (ref 459fd9a7, corpus 5a7a8be9). pooled-same **null_contrast N 82.3 / E 91.2** vs **control N 88.3 / E 93.7**; paired Δ **N whole −6.0** (unseen −8.9, **zs +3.7**), **E whole −2.5** (seen −5.8, **zs +1.3**); ref-dep gap N +20.6/+22.7, E +27.6/+32.6; **near_copy 0/152** all four; core_degenerate null 10/3 vs control 5/0. Co-scored control N=88.3 ≈ published 89.6 (evals/012, seeds 42/43) ⇒ pipeline validated. **NEUTRAL:** the lerp-null contrast term did NOT beat the plain control on whole (deficit in seen/unseen), while zero-shot rose → owner/FINDINGS, not settled. ⚠ **eval renumbered 024→026** — a concurrent session had taken evals/024-025 for the DCG sweep (I checked max=023 before both registered); gens unaffected (mine 028, DCG 029-032). `store/evals/026_dualforce_null_contrast__dai__2026-09-03` (meta+HEADLINE), gens/028 scores linked, fsck PASS. Aggregator `report_nullc.py`. Viewer next. [UNCERTIFIED]
- `03:03` **DCG sweep on the dual-force plain-FM control (runs/012@1000).** Ran the ctt_v2 DCG protocol (test-time Demonstration-Contrastive Guidance, crossfade null, gs=4/stg=1, w∈{1,1.5,3,6}, seed 42, 152 roster) adapter-swapped onto dualforce_control 1k. 1216 clips, 0 failures; eval-v4 app%same neutral 94.1/96.5/96.4/99.2, effect 100.0/100.9/102.0/100.9 (effect ceiling-saturated). Registered store gens/029-032 + evals/024-025 (NUMBERS ONLY — demo-copy guards pending). Fixed `run_gen.py` passing `signal_id=None` to the ctt-v2-train ReferenceConditionConfig (extra_forbidden; DINO-signal leftover) — now omitted when no SIGNAL_CONFIG. Campaign misc/2026-09-02_dcg_dualforce_control/DOSSIER.md.

- 02:18 **dualforce `null_contrast` (control_null) — trained to step 1000, registered, gen launched.** The folded "control + lose" arm shipped @1000 on bgms (jobs 3073199-3073201, 2 GPU × accum4 = **eff-batch 8** = 012's), ckpt sha **392b4d4e**, **960 tensors / 0 signal keys** (plain IC-LoRA — architecturally identical to ctt_v2/012, so the proven one_way inference stack). Staged `store/runs/020_dualforce_null_contrast` (config+meta+ckpt); registered `eval_ladder/arms.yaml` (`dualforce_null_contrast_{neutral,effect}`) + `store/ARMS.md` + INDEX; stamped registries via `stamp_rows.py` over families 001/002 (152 rows each; prompt_sha neutral **0d708175**, effect **35930d7d**). Gen launched: seed 42 × 152-grid, neutral+effect, `run_gen.py` one_way `--rank 128 --alpha 128` → `store/gens/028_dualforce_null_contrast/{01_neutral,02_effect}__dai` (arrays 3075409/3075410 on bgms, 8 chunks each, skip-if-exists). NEXT (loop): v4 eval **co-scoring dualforce_control@1000** for the certified A/B, then `iclora_neutral_effect` viewer. Campaign `misc/2026-09-02_null_contrast/`. NEUTRAL — verdict → owner/FINDINGS. [UNEVALUATED]
- 01:14 **DINO-signal arm A6_tokens_sigonly prepared, gated, committed — launch BLOCKED on account availability.** New arm (advisor pre-reg `misc/2026-08-27_dino_signal_training/A6_PREREG.md`): the A2_tokens arm with the raw pixel reference demo dropped EVERY step (`reference.probability: 0.0`), so the 44-ch DINO signal (A2's pseudo-latent bank tokens, one-way read-only block) + endpoints/mask + text are the only conditioning — n_ref==0, layout `[sig | target]`, 6000 steps. Tests H1 (44-ch code inadequate) vs H2 (code redundant with the pixel demo) for the 09-02 "signal unread" result. One backward-compatible inference-port change in `src/LTX-2-armA` @ **f53c8a6** (`validation_runner._signal_for_sample` + `config.ValidationSample.signal_id`): a sample may carry `signal_id` WITHOUT a reference and receive the REAL signal; reference samples unchanged (bit-identical); xattn signal-only raises. No new training code (existing `ReferenceConditionConfig.probability` draw + `_signal_attach_bank` handle n_ref==0). CPU gates PASS: `verify_signal_store`, `assert_pins`, new `build/gate_a6_cpu.py` (training layout at n_ref==0 for S0/S1/S4 + both S6 orientations; inference port; backward-compat; null control; probe path). Config `configs_005/A6_tokens_sigonly.yaml` (diff vs A2 = only reference.probability 1→0, steps 10000→6000, 3 validation samples reference→top-level signal_id, per-arm output_dir/tag). **Launch blocked:** the directed DeltaAI account `bgjg-dtai-gh` is no longer associated with the user (`sbatch --test-only` → "Invalid account or account/partition combination"); it charged our A1/A5 chains on 08-29/30 but the association was since removed, and the task forbids the alternatives (bhwp reserved/busy, bgms "never"). Everything is one command from launch once the account is restored — see `A6_STATE.md` (phase=BLOCKED, exact error + command). Campaign files under git-ignored `/misc/` (durable on $LAB).

## 2026-09-02

- 15:05 **Switched the null-contrast arm to the folded `control_null` objective (`e938cd4`) before it ran.** Owner: it should be "exactly the control arm + a lose objective with the lerp as reference." New `contrast_axis=control_null`: the primary dataloader row IS the win, so **one forward yields both the control's plain-FM loss (L_FM, normal sampler + full mix — unchanged) and l_w**; a second item = same row/target with reference := LERP-NULL(its own demo) gives l_l. `loss = L_FM + λ·L_con(l_w,l_l)`, `ref_anchor=false` (SimPO — self-anchored: L_FM pins l_w low, so the term only pushes l_l up; can't be gamed by uniform degradation). Drops the separate `matched` forward and the DPO ref forward → **1 grad forward (batch-2 [win|lose])**, ~2× control's per-step cost instead of ~4–5×. No cell pair / band sampler; contrast rides the primary row's own noise/mix so the base is byte-for-byte the control. CPU-verified (compile, AST method+name resolution, full `LtxTrainerConfig` validate). Relaunched 4 GPU × accum 2 = eff-batch 8 (fresh dir `null_contrast_ctrllose`, jobs 3073040–42). Prior eff-batch-8 `demo_null` chain (3072688–90) never got a slot → cancelled, nothing lost.
- 14:20 **Temporal-dynamics probe — λ sweep for the OT channel** (owner ask; POST-HOC below the pre-registered grid). Job 3072913: λ ∈ {0, 0.05, 0.1, 0.25, 0.5, 1, 2} × unit/mag × core/window on the frozen exam and as capped pooled-% on the 20 arms. Exam accuracy of the composite kernel, core/unit: 0.8117 / 0.8251 / 0.8475 / **0.8655** / 0.8386 / 0.7982 / 0.7130 (mag peaks 0.8610 at 0.25 too) — λ=0.25 is an interior optimum, resolving the grid-edge caveat (re-test still owed before adoption); no-core never beats the control (best 0.8072). On gens every arm rises with λ, externals fastest (refVFX B 31 → 55, VAP neutral 44 → 62 from λ=0 to 2 vs trained arms +2–3 pp), so the trained-vs-external spread shrinks 56 → 33 pp: larger λ rewards when-the-look-changes over what-changes. `misc/2026-09-02_temporal_dynamics_metric/LAMBDA_SWEEP_RESULTS.md`.
- 13:40 **Temporal-dynamics probe — capped pooled-% + per-row dump** (owner ask: per-gen pool-% capped at 100 before averaging; some gens reach 158–159%). Job 3072645 (6 min 28 s, 20 arms, all cached). Core S3 pooled-% same uncapped → capped: ic_gen 83.1 → 80.9, ctt_v2 82.9 → 82.1, dualforce 89.5 → 87.3, DINO neutral 84.5–85.9 → 83.0–83.8, DINO effect 86.8–90.1 → 85.0–87.9, externals unchanged (0–2% of gens above 100%). Cap bites only on strong 121-frame arms (share of same-gens >100%: dualforce 26%, DINO effect 28–38%, ic_gen/ctt_v2 17%), compressing the top of the table. Per-row unit similarities for all 26,754 rows now in `outputs/eval/temporal_dynamics/breakdown_rows.jsonl` (re-aggregation = CPU one-liner). `BREAKDOWN_ALL_RESULTS.md` carries capped and uncapped tables.
- 14:35 **`demo_null` run — two corrections after launch.** (1) **Bug fix `4c87e15`:** the step-0 crash (`'dict' object has no attribute 'dim'`) was because batch `reference_latents` is a dict `{latents:[B,C,F,H,W], num_frames, height, width, fps}`, not a bare tensor; `_lerp_null_reference` now unwraps/dissolves/rewraps (verified vs a real bf16 ctt_v2 ref: endpoints preserved, keys intact). Fix confirmed on GPU — 2-GPU trial reached step 43 clean (`l_w 0.2549 < l_l 0.2799`, `eps_gap 0`, Δ −1.4e-4). (2) **eff-batch corrected to match the control:** the first launch cloned 016's **2 GPU → eff-batch 4**, but the `012` control is **eff-batch 8 (4 GPU × accum 2)** — a confound for the @1000 comparison. Relaunched at **4 GPU × accum 2 = eff-batch 8** (fresh dir `outputs/dualforce/null_contrast_eb8`, jobs 3072688–90), which also ~halves wall-clock (~3.3 h vs 6.4 h; contrast step ≈4 forwards vs plain-FM's 1, so slower per step than the control — 016 took ~5.8 h on 2 GPU). Dataset unchanged (`datasets/ctt_v2` — identical to 012/016).
- 13:42 **`demo_null` contrastive arm — LERP-dissolve reference as the DCG null; launched on DeltaAI.** New `twin.contrast_axis='demo_null'` in the `contrast` fork (`src/LTX-2-contrast` @ **b5d7e0b**): win=(state A, real demo A)→v_A vs lose=(state A, **LERP-NULL(demo A)**)→v_A, where the null reference is an on-the-fly latent-space dissolve of demo A's own endpoints (first↔last latent frame, linear cross-fade; new `_lerp_null_reference`) — the DCG null moved into training as a within-arm reference-reading contrast; needs no partner row; existing `sample`/`demo` axes untouched (backward-compatible). Everything else **IDENTICAL to the 012 control / 016 contrast recipe**: warm-start ctt_v2 `002@10k`, r128/α128 attn_ffn one-way, WSD 2000-step config **shipped @1000**, lr 1e-4, β 8.0 / λ_con 0.25 / warmup 250, σ∼U[0.5,0.9], ref-anchor = frozen `002@10k`, S0+S1 cell index, ctt_v2 data. CPU pre-flight PASS (py_compile, lerp unit test = endpoints preserved + linear cross-fade, full `LtxTrainerConfig` validation, inputs present). Resumable singleton chunk chain **`nullcon` (jobs 3071771–73, bhwp, 2 GPU × accum 2, `--time 2:30`, ship @1000)** submitted; compares against `dualforce_control@1000` (89.6). Assets `misc/2026-09-02_null_contrast/build/{df_null_contrast_chunk.yaml,train_null_contrast_chunk.sbatch}` (`/misc/` git-ignored per campaign convention). NEUTRAL — a research bet: 016's cross-operator contrast was a negative (78.5), and the dissolve null may saturate (far from any operator); result → owner/FINDINGS, not settled.
- 13:12 **Temporal-dynamics probe — per-unit breakdown extended to 20 arms** (owner ask: externals + DINO arms). Job 3071477 (7 min 47 s; refVFX A/B, VAP and VFXMaster neutral/effect/authorcfg, DINO A0/A1/A2/A5 neutral+effect added; ~1,050 external clips re-extracted into the private cache). Anchors reproduce stored numbers (refVFX pool-% all 41.7/29.2; DINO pooled-same ≤0.1 pp). Core family pooled-% same, units P1 / P2 / V1 / V1e / OT-X / S3: refVFX B 64.1/38.2/20.2/12.0/25.3/33.0; VAP neutral 66.1/47.6/36.4/31.7/42.0/44.7 (authorcfg 81.9/69.2/63.6/56.5/59.9/67.5); VFXMaster neutral 66.3/53.2/40.7/36.1/49.6/48.2 (authorcfg 82.4/69.9/64.7/56.7/63.7/68.4); DINO A1 effect 89.6/87.5/90.9/90.9/96.2/89.6; A5 effect 91.0/87.4/91.5/90.6/93.2/89.8. Externals' deficit sits in the dynamics units and their appearance credit is endpoint-borne (P1→P2 −13 to −27 pp vs −2 to −6 for trained arms); 33/49-frame clips ranked against the 121-frame corpus population — dynamics not strictly comparable across T. `misc/2026-09-02_temporal_dynamics_metric/BREAKDOWN_ALL_RESULTS.md`.
- **13:07** — DINO-signal eye test, two measurements: (1) sampling-time swap sensitivity on the 63 rendered pairs — matched-vs-deranged PSNR A1 17.2 dB / A2 21.8 / A5 23.8 (middle frames; same-row A0 ≈ 15.5 dB), i.e. the deranged signal changes the sample substantially; (2) determinism control — 16 matched rows re-rendered on other GH200 nodes are BIT-IDENTICAL (138 dB), so the differences are the signal alone. Read together with the flat loss probe this is consistent with chaotic amplification of a content-blind perturbation; the discriminating test (toward-donor scoring) is not yet run. Viewer `dino_shufsig_eye` now shows a per-row swap-PSNR chip. Script misc/…/eval/swap_sensitivity.py.
- 12:45 **Temporal-dynamics probe — per-unit breakdown of the m1a/S3 kernel on the four arms** (owner ask). CPU-from-cache job 3071362 (1 min 25 s). pooled-% same per unit, base+cond / ic_gen / ctt_v2 / dualforce control, core family: P1 70.9/86.4/85.7/89.6 · P2 59.1/80.2/81.4/87.8 · App 65.1/84.0/84.0/89.1 · V1 56.4/83.5/81.1/91.7 · V1e 55.4/83.8/81.3/90.6 · Dyn 55.9/83.7/81.2/91.2 · OT-X(λ=0.25) 60.0/85.4/83.8/91.0 · S3 60.4/83.1/82.9/89.5. No-core family: P1 72.5/86.9/87.9/90.9 · V1 63.9/82.9/84.2/87.8 · V1e 64.3/82.5/84.7/87.8 · OT-X(mag) 55.4/82.8/84.4/87.4 · S3_win 64.6/83.3/84.8/88.5. Arm spread (dualforce−base+cond): P1 19 pp, P2 29, V1/V1e 35, OT 31 (core); without core the velocity units' spread collapses to 24 pp while magnitude-weighted OT keeps 32. `misc/2026-09-02_temporal_dynamics_metric/{BREAKDOWN_RESULTS.md,breakdown.py}`.
- **12:37** — DINO-signal: read the pre-registered probe reading bar (`probe/delta_deranged`, DOSSIER 2026-08-29) from wandb for every 005 arm — ≈0 throughout training for A1 channels / A2 tokens / A5 xattn while `signal/contrib_ratio` grew to 0.49 / 0.90 / 1.31: no arm reads the signal's content (bar FAILED; pre-registered follow-up = reference dropout p≈0.3). Rendered the deranged-signal EYE TEST for A1/A2/A5 (21 zs+unseen same-class rows each; same pixel reference, endpoints, prompt, seed — only the fed 44-ch signal swapped, rotation-by-2) into store gens 027/024/026 `03_neutral_shufsig__dai` (unregistered) and built the tabbed flip viewer `dino_shufsig_eye` (`scripts/viewers/gen_dino_shufsig_eye.py`). New arms `dino_a{1,5}_*_shufsig_neutral` (arms.yaml, ARMS.md aliases); generic DINO gen job takes an optional `CELLS` filter.

- 12:35 **Temporal-dynamics probe, Stage C — four arms re-scored under the candidate kernels** (owner ask: pooled-% same with the no-core temporal-OT kernel for base+cond, ic_gen, ctt_v2, dualforce control plain-FM). One DeltaAI job (3071254, 4 min 18 s, ≈0.1 SU): each arm's stored v4 gen×pool-ref rows (1,842 / 1,842 / 1,817 / 1,769) re-scored on one machine with shared 222-corpus statistics, now persisted (`outputs/eval/temporal_dynamics/corpus_matrices_222.npz`) so later kernel passes over cached arms are CPU-only, seconds per arm. Control reproduces every stored app_ref exactly (max|Δ| 0.0 on all four arms incl. 160 re-extracted gens) and each stored headline (60.42/83.13/82.95/89.56). **pooled-% same, S3 → core-OT λ=0.25 → S3 no-core → no-core OT:** base+cond 60.43 → 62.12 → 64.59 → 60.03; ic_gen 83.06 → 84.32 → 83.33 → 83.31; ctt_v2 82.92 → 83.86 → 84.82 → 84.64; dualforce control 89.53 → 89.58 → 88.53 → 88.17. Arm ordering unchanged under every kernel; no-core kernels lower the foreign cells (−9 to −11 pp) and shrink the G-fit − G-ref-control gap in every reference arm (ic_gen 17.5→14.7, ctt_v2 18.1→13.4, dualforce 21.4→17.3 under no-core OT); core OT raises foreign cells (+5 to +9 pp). Same caveats as Stage B; nothing deployed. `misc/2026-09-02_temporal_dynamics_metric/{STAGE_C_RESULTS.md,REPORT.md}`.
- 11:56 **Temporal-dynamics appearance-metric probe** (owner question: the m1a pooled-% dynamics half is an order-agnostic bag of CLS velocities — does an order-aware channel do better, does the core mask matter?). Pre-registered (`misc/2026-09-02_temporal_dynamics_metric/PREREG.md`), CPU numpy/scipy over cached DINO CLS, two DeltaAI jobs ≈0.2 SU, nothing deployed. **Stage A (frozen 223-clip exam, judge imported unchanged; base touch: rebuilt m1a__v3_sided reproduces the pinned exam exactly, S3 max|Δ| 4e-05 = one ECDF rank step of aarch64-vs-x86 ulp noise, OT(λ=0)≡V1e to 9e-12):** control S3 acc 0.8072 / d 1.734 / 43 mis. Replacing Dyn(V1,V1e) by exact temporal OT over unit CLS-velocities with cost (1−cos)+λ·|Δτ|: **λ=0.25 → acc 0.8655 (+13 clips), d 1.960, 30 mis, hubness PASS**; λ=0.5 0.852; λ≥1 ≤ control; strict order (DTW / aligned cosine) 0.34–0.62 standalone. No-core: S3_win 0.7354 (−16), best core-free candidate ties the control (0.8072). λ=0.25 is the grid edge — flagged, not tuned further. **Stage B (ctt_v2 plain/neutral, the 1,817 stored evals/008 rows, 222 corpus + artifact 459fd9a7; control reproduces stored app_ref with max|Δ| 0.0 on every row and the 82.95 headline):** pooled-% same 82.92 (S3) / 83.86 (core OT λ=0.25) / 84.82 (S3 no-core) / 84.64 (no-core OT); pooled-% all 73.6 / 76.4 / 71.3 / 72.3 — the core-OT gain sits in foreign/cross cells (G-unseen-foreign 59.2→66.7, G-zs-foreign 54.6→60.0), no-core lowers them (→47.6/47.5); G-fit − G-ref-control gap 18.1 / 16.2 / 12.3 / 13.4 — every alternative shrinks the measured reference-dependence; ρ(sim, core_frames) +0.30 for core kernels vs +0.10–0.14 for window kernels. NOT done: the hold/lerp control-arm degeneracy read (008 cache gone, manifests absent → would need re-synthesis + GPU). The kernel is a pinned instrument constant — adoption = finer pre-registered λ grid + degeneracy read + version bump + re-cert. REPORT.md / STAGE_A_RESULTS.md / STAGE_B_RESULTS.md in the misc folder; matrices in `outputs/eval/temporal_dynamics/`.
- 07:42 DINO-signal **A1 channels-target** — full production pipeline END-TO-END on DeltaAI, mirroring the A5/A2 close-outs. The route-aware `SIGNAL_CONFIG` hook (b124df3) already handled `channels` generically, so NO code change was needed; wrote `gen/cfg/a1_channels.json` (route=channels, placement=target, channels 44, hidden 256, token_pool 2, guidance 1.0, checkpoint = the staged A1 adapter). Fail-fast confirmed the port attaches **route=channels, 1/1 signal.* key strict (`proj.weight`)**, norm db47be88, `signal_id=eval__animalization_1` (matched own ref), n_target 4800, clip 480×640×121. Registered arms `dino_a1_channels_{neutral,effect}` (arms.yaml, ARMS.md slug `dino_a1_channels_target`, adapter = the staged `misc/.../staged_adapters/A1_channels_target` 10k **sha 1093362b**, 961 tensors = 960 LoRA + 1 signal.proj.weight, dataset **005** — SAME as A2/A5). Gen: full 152-row grid × seed 42, both prompts, MATCHED only (neutral bhwp / effect bgms; bgjg not associated for this user on ghx4) → `store/gens/027_dino_a1_channels/{01_neutral,02_effect}__dai` (store_register + store_fsck PASS, `scores` symlinks into evals/023). Scored v4 into **new** entry `store/evals/023_dino_a1__dai__2026-09-02` (reference sha 459fd9a7, torch 2.10.0+cu129, one pass, 16 chunks/arm): pooled-same **A1n 84.8 / A1e 89.6**; ref-dep gap **+29.5 / +33.7pp** (G-fit 92.9/95.8, G-ref-control 63.4/62.1); seen/unseen/zs **92.9/91.1/81.5** (n) · **95.8/100.4/86.5** (e); **near_copy 0/0**; core_degenerate 10/5 of 152; copy_max 0.3915/0.4499; err rows 30/20 of 921/arm. Comparators (same-machine cross-pass): A5n 84.9 / A5e 89.8 / A2n 86.0 / A2e 90.1 / A0n 84.6 / A0e 86.8 / ctt_v2 82.5. Integrated into the flagship `iclora_neutral_effect` page under the existing **dino_signal** category (2 EXTERNAL + 2 CATALOG, manifest-shape `viewer_manifest_s42.jsonl` per subentry); all seatbelts passed, viewer LIVE :8017. NEUTRAL numbers — **A1-vs-A2-vs-A5 is a CLEAN same-dataset (005) ROUTE comparison** (channels vs tokens vs signal-as-Q xattn; A1-vs-A0 stays 004-confounded); NO pre-set bars (bar-setting consult owed); the within-arm A1-matched vs A1-shufsignal contrast is not in this matched-only pass. gen num_inference_steps 30 (arms.yaml global) vs 20 at training-validation — sampler knob, flagged for owner (same as evals/021-022).

## 2026-09-01

- 23:07 DINO-signal **A5 xattn-fusion** — full production pipeline END-TO-END on DeltaAI. Extended the `SIGNAL_CONFIG` hook in `eval_ladder/run_gen.py` to build the `SignalModule` route-aware (threads `channels`+`heads` from the config so xattn is byte-identical to `Trainer._signal_attach`; tokens/channels unchanged); fail-fast confirmed the port attaches route=xattn, **13/13 signal.* keys strict**, `signal_id=eval__<own ref>`, clip 480×640×121. Registered arms `dino_a5_xattn_{neutral,effect}` (arms.yaml, ARMS.md slug `dino_a5_xattn_fusion`, adapter = the DeltaAI-visible `outputs/dino_signal_005/A5_xattn_fusion` 10k sha 584997b6, dataset **005** — SAME as A2, so A5-vs-A2 is NOT dataset-confounded). Gen: full 152-row grid × seed 42, both prompts, MATCHED only (neutral bhwp / effect bgms; bgjg is not associated for this user on ghx4) → `store/gens/026_dino_a5_xattn/{01_neutral,02_effect}__dai` (store_register + store_fsck PASS). Scored v4 into **new** entry `store/evals/022_dino_a5__dai__2026-09-01` (reference sha 459fd9a7, torch 2.10.0+cu129, one pass, 16 chunks/arm): pooled-same **A5n 84.9 / A5e 89.8**; ref-dep gap **+26.4 / +32.7pp** (G-fit 90.3/95.0, G-ref-control 63.9/62.3); seen/unseen/zs **90.3/99.5/82.0** (n) · **95.0/101.4/92.2** (e); **near_copy 0/0**; core_degenerate 10/7 of 152; copy_max 0.3798/0.4406. Comparators (same-machine cross-pass): A2n 86.0 / A2e 90.1 / A0n 84.6 / A0e 86.8 / ctt_v2 82.5 / control 012 89.6. Integrated into the flagship `iclora_neutral_effect` page under the existing **dino_signal** category (2 EXTERNAL + 2 CATALOG, manifest-shape `viewer_manifest_s42.jsonl` per subentry); all seatbelts passed, viewer LIVE :8017. NEUTRAL numbers — NO pre-set bars (bar-setting consult owed); the claim-bearing within-arm A5-matched vs A5-shufsignal contrast is not in this matched-only pass. gen num_inference_steps 30 (arms.yaml global) vs 20 at training-validation — sampler knob, flagged for owner (same as evals/021).
- 18:12 DINO-signal arms — PRODUCTION close-out (last mile). Scored the 4 MATCHED-case arms into `store/evals/021_dino_signal__dai__2026-09-01` (v4, DeltaAI GH200, torch 2.10.0+cu129, reference sha 459fd9a7, seed 42, 152 rows/arm): pooled-same **A2n 86.0 / A0n 84.6 / A2e 90.1 / A0e 86.8**; ref-dep gap +33.0/+34.1/+27.1/+29.8pp; near_copy 0/1/0/0; core_degenerate 10/7/6/6. Store-registered gens **024_dino_a2_tokens** + **025_dino_a0_baseline** (4 subentries, `store_register.py`, `store_fsck` PASS) with `scores` symlinks into evals/021. Integrated into the flagship `iclora_neutral_effect` page under a new **dino_signal** category (4 EXTERNAL entries + 4 CATALOG rows); the single-seed grid needed the manifest-shape `viewer_manifest_s42.jsonl` per subentry (152 clips, not the registry-shape 2-seed 304 the seatbelt demanded — the DCG single-seed precedent). NEUTRAL: **A2-vs-A0 is DATASET-CONFOUNDED** (A0=004, A2=005), NO pre-set bars (bar-setting consult owed), and the claim-bearing within-arm A2-matched vs A2-shufsignal contrast is not in this pass (shufsig registry stamped + ready). DOSSIER "PRODUCTION CLOSE-OUT" appended.
...
- `12:24` S6 reshape (dataset 005, advised Round 6) — **assembler now stamps `samples_sha256`/`samples_rows` NATIVELY** + count-derived `shapes.note`. Additive edit to `scripts/ctt_v2/assemble_root.py` code-side branch (`samples_sha256 = rc.sha256_file(samples.jsonl)`, `samples_rows = n`, added to ROOT_MANIFEST before write; `shapes.note` = `f"the root holds {len(shape_counts)} shape classes; …"`) — assemble_root.py is the ONLY tracked file changed. Plan-only regressions EXACT (003 S6 57,847 / 2,378 dropped / total 114,215 / max_dev 0; 005 S6 81,779 / 92 / 138,147 / max_dev 0). **Determinism proof:** a full code-side re-assembly of 005 into a scratch root (ROOT5 never rewritten) reproduced `samples.jsonl` sha **048d1ef4** / 138,147 rows; mix/VERSION/CAPTIONS/5 masks == root5_small; manifest key set equal to ROOT5's, values equal except {created, root, mask_store.dir, shapes.note}; native `samples_sha256`/`samples_rows` present and equal; `_shape_cache.json` unchanged on the 100% hit. `build/watch_all.sh` output roots now derive from CFGSET (`OUTROOT=…/outputs/dino_signal${SET:+_$SET}`, eps `outputs${SET:+_$SET}`, SET passed into the eps ssh) — report-only verified for configs_005 (dino_signal_005/outputs_005) and configs_004 (dino_signal_004/outputs_004); `deltaai_chains.tsv` stays all-commented. eps ship documented in store 005 (README/CODESIDE_FORMAT eps section + `_shape_cache` note + native-stamp history; meta.yaml `eps_ship_2026_08_30` append-only block); `store_fsck` clean. `REPORT.md` written (spec, per-stratum/shape, costs bhwp 7.87 / bgjg 8.52 GPU-h, verification both machines, limitations). 0 GPU-h; eps write-free (one read-only census). Store 004 / norms / native EFFECTDATA / trainer fork / ROOT4 / ROOT5 UNTOUCHED.
- `10:40` S6 reshape (dataset **005**, advised Round 4) — **code-side root 005 + store entry + configs_005** (NO GPU). Additive contract-keyed edits to `scripts/ctt_v2/root_common.py` (MIX_CONTRACTS `005_ctt_v2plus_s6reshape`, weights IDENTICAL to 003) and `scripts/ctt_v2/assemble_root.py` (CONTRACT_ROOTS/_CONTRACT_PRESENT/CONTRACT_S6_ROSTER/CONTRACT_S6_INVENTORY/CONTRACT_VERSION dicts + a manifest-contract guard + keyed S6-roster read); 003 regression exact (plan-only reproduces S6 57,847 / total 114,215 / max_dev 0). New `scripts/ctt_v2/s6/derive_inventory_r832.py` → `S6_r832.json` (28,644 clips, 57,288 paths exist). Root **`outputs/ctt_v2/roots/ctt_v2plus_s6reshape_mix`**: total **138,147** (S0 385 / S1 3,675 / S2a 22,731 / S2b 23,577 / S4 6,000 / **S6 81,779**, 92 same-grid singletons dropped, 2,286 returned), shapes corpus (16,20,15)=50,368 / s4 (5,14,26)=6,000 / **(11,16,26)=41,592 / (11,26,16)=40,187** (4,576 tok each), 5 masks, VERSION `3.1.0-ctt_v2plus_s6reshape-codeside`, `samples_sha256 048d1ef4…`. Verification (all PASS): `verify_code_side_005.py` invariants 1-5 (S6 same-grid + different-subject on all 81,779; existence FULL over 100,301 paths); `compare_004_005.py` (non-S6 rows multiset-identical to 004, id unchanged; mask frame-0 sum 416); `validate_training_ready_005.py` V1 100% / V6 eval 223 / V8 NORM_dino_v4 smoke; `signal_fsck` 100% every stratum; `verify_signal_store` PASS ×5 arms (version dino_signal_norm_v4). `store/datasets/005_ctt_v2plus_s6reshape/` (README + CODESIDE_FORMAT + BUILD + meta.yaml + root) + INDEX row 5; `store_fsck` clean. `configs_005` (gitignored) rendered by `build/make_005_configs.py` (signal.root = `cache/armA_signals_005/feat`, NORM_dino_v4 db47be88…, MIX line byte-identical to configs_004). Store 004 / norms v1–v3 / native EFFECTDATA / trainer fork / eps / ROOT4 UNTOUCHED.
- `09:24` S6 reshape (dataset 005, advised Round 3) — **NORM_dino_v4 by addendum** in `store/datasets/003_dino_signals/` (sha256 db47be88…): refit over S0,S1,S2a,S2b,S4,S6 from the new signal root `cache/armA_signals_005` (non-S6 feat = hardlinks of 003; S6 = 28,644 fields re-extracted at r832 grids 832×512/512×832). Full S6-r832 DINO extract array (job 3049605, bgjg, 96/96, Σ 7.69 GPU-h). Gates G-N1..G-N5 PASS (asinh/dead none); n_files 47,444; S6 n_cells 131,074,944; non-S6 per-stratum moments identical to v3 (relΔ 0). PCA health (pre-registered bars, all PASS): pooled 0.3744 (≥0.339); 4 native shapes 0.3703–0.3781 (≥0.316); META-vs-V4-raw cross-check max|Δ| 3.7e-15 (≤1e-6); paired r832/native ratio pooled 0.9694, per-native 0.9645–0.9732 (≥0.90/0.88). `fit_norm_dino.py` given default-preserving `--feat-dir/--pca/--s6-roster/--s6-coverage-rows` flags (v3 behaviour byte-identical). Store 004 / norms v1–v3 / native EFFECTDATA / trainer fork / eps untouched.
- `03:40` S6 reshape (dataset 005) Round 2: added `scripts/ctt_v2/s6/encode_array_r.sbatch` (48-task GH200 array re-encoding S6 to the two orientation grids 832x512/512x832; job 3049217).
- 03:16 S6 reshape (dataset 005, advised Round 1) — RULED_SHAPES extended in `scripts/ctt_v2/root_common.py` with the two reshaped S6 grids: `effd_832x512_81f` (11,16,26) for landscape-native and `effd_512x832_81f` (11,26,16) for portrait-native, both 81f/24fps, 4,576 tokens (95.3% of corpus 4,800), shift 2.222, prefix_latents 1 (frame-0 anchor). Additive only (11 insertions); 004 plan-only reproduces exactly (S6 57,847 pairs, total 114,215, max_dev 0.0000). CPU pre-computation (misc/2026-08-30_s6_reshape): derived roster 28,644 (L 14,523 / P 14,121); VAE-vs-DINO crop windows agree ≤1.318 source px; orientation-grid pairing gives 81,779 pairs and returns 2,286 of 004's 2,378 shape-singletons (92 remain). GPU pilot (bhwp, job 3049192, 1m37s): 24 clips (6/native-shape) encode+verify PASS, decode frame-0 PSNR 30.2–36.3 dB (bar ≥26) on all 4 crop cases. New sibling drivers `scripts/ctt_v2/s6/encode_s6r.py` + `pilot_r.sbatch` (native EFFECTDATA/ + store 004 + trainer fork untouched).
- 02:30 DINO-signal arms — owner pivot: the four DeltaAI `configs_004` chains were cancelled at step ≈ 20 (06:45 UTC; eps A0 on 004 keeps running) and an ADVISED Opus-4.8 operator (Workflow `wf_a397329d-582`, fable advisors at every fork) was launched to build the S6-reshaped successor dataset — re-encode S6 at advisor-chosen grids, re-extract the DINO signal, norm v4 by addendum, code-side root, store entry 005, `configs_005`, eps ship, docs, then a final Fable check (brief: misc/2026-08-30_s6_reshape/BRIEF.md). Relaunch of 4 DeltaAI arms + eps on 005 follows.
- 01:52 DINO-signal arms — measured (wandb `_runtime`): eps A0 10.1 s/step steady (validation ≈ 1.5 min/500 steps ⇒ ~28.5 h/arm; the trainer's "Total Time" is the REMAINING estimate incl. step-0 overhead), DeltaAI A1 ≈ 21 / A2,A5 ≈ 26 s/step. Found that 3h10 chunks with a 250-step checkpoint interval fit ONE checkpoint ⇒ walltime kills would discard up to 250 steps per chunk (~42 %). Fix in `build/train_chunk.sbatch` (fork untouched): chunks self-stop at resume_step+500 once the `Step N/` line ≥ STOP_AT+20 appears (probe/W&B flushed), SIGTERM the trainer's process group, exit 0; walltime 4h30 as safety net. 28 dependency-blocked chunks resubmitted (7/arm); first four chunks stay on the old script and are cut after their step-250 checkpoint (`ONE_OFF_ACTIONS.md`, flagged by the watcher).
