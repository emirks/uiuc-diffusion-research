...
**12:29 — First full draft of the ICLR 2027 paper written (`papers_drafts/ctt_iclr2027/`, untracked in this repo).** Every section now has prose (abstract, intro with refutable contributions, related work, task with the lerp-collapse measurement, method with reading + three control axes, dataset, evaluation, experiments with table readings, ablations, conclusion, statements); 16 pages total including references and the five appendix tables; builds with 0 errors, 0 undefined `\ctnum` keys, 0 overfull boxes. Writing rules were first distilled from primary sources into `WRITING_GUIDE.md` (Peyton Jones, Widom, Nanda, Lipton & Steinhardt, Gopen & Swan, the ICLR 2027 reviewer guidelines), and the prose was written from three agent-extracted fact sheets with file:line pointers (`facts/`), not from memory; numbers outside the metric-table generator are centralized in `tables/manual_numbers.tex` with their source record so no number is typed in a section. Thirteen `\gap{}` boxes state what the paper does not yet have (teaser/method/dataset/qualitative figures, the champion's deranged-reference control, the prompt-escape test, a user study, dataset/method ablations, the dataset-size decision) and two story-versus-record differences are surfaced rather than smoothed: localized content control is prompt editing (the attention-scoping route was an architectural negative), and the 2026-08-06 metric audit's per-item findings are disclosed next to the pooled headline.
