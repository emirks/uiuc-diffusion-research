...
- `<ns>.json` — the sidecar meta:
  ...
  where `origin` is `"extracted"` or `"migrated:<legacy file path>"`.
...
eps↔DeltaAI at the 0.005 bar); `fsck` warns when one variant mixes hosts, and an eval's `meta.yaml` names the host it scored on.
