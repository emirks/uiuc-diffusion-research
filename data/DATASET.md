...
| S2a render (7,990 clips / 799 ops) | **FROZEN** | `S2_ACCEPTANCE.json` verdict PASS, §5.4. ⚠ **Acceptance certifies STORAGE, not consumption** — read the §5.4 acceptance annotation before quoting 7,990 as what the root trains on. |
...
| The 8 pre-registered S2a inline-OOD ops | **FROZEN (pre-registered)** | `PREREG_inline_ood_ops_s2a.json`, advisor-ratified 2026-07-28, §11.4. 🔒 **Never re-drawn.** The **demo** pool derived from it is **79** (not 80) per `A18_28plus1_and_ood_demo_VERBATIM.md` — `DERIVED_inline_ood_demo_pool.json`, §5.4. |
...
| **Acceptance annotation (A18)** | 🔴 **Verbatim, and it is the only permitted wording:** *"Render acceptance covers 7,990 S2a clips on disk; assembly consumes 7,577 clips / 22,731 base pairs after ALL recorded drops (census: 28 role-scoped clips per A16/A17, 305 clips under standing holdout reasons, 8 inline-OOD groups = 80 clips). All 29 consumers of the excluded (`openvid_T1MiFx98l3g_0_50to156`, A) pair are dispositioned: 28 clip-dropped, 1 via its group's `inline_ood_op` drop. Evidence: ROOT_MANIFEST drop record per the A18 amended closing evidence."* — `A18_28plus1_and_ood_demo_VERBATIM.md`. **Acceptance certifies the deliverable (storage); `assert_root` certifies consumption.** `S2_ACCEPTANCE.json` is **byte-untouched and NOT re-run**. ⛔ **Never write "the root consumes 7,961"** — A18 forbids it explicitly: it subtracts only the role-scoped drops and ignores the standing holdout/OOD drops, **no artifact will ever show 7,961**, and writing it plants the next absence-read-as-success mine. ⛔ **Never transcribe pp figures** (neither A16's 34.437/34.563 nor the rehearsal's 35.8332/37.1668) — the manifest's derived-at-assembly value is the only record (`A12_prorata_s2_split_VERBATIM.md`). |
...
| **Holdouts** | 10 shader families never rendered (§7). **8 inline-OOD ops pre-registered and excluded at assembly** — `PREREG_inline_ood_ops_s2a.json`, seed 42, 8 distinct shaders, 80 clips (~1 % of S2a); their encodes stay on disk for the inline lane. §11.4. **The OOD *demo* pool is 79, not 80** (`A18_28plus1_and_ood_demo_VERBATIM.md` decision 2): `s2_0818_c03` consumes the A10-excluded pair and is excluded from the demo lane too, derived mechanically as `clip_ids ∩ consumers` → `misc/ctt_v2_final/DERIVED_inline_ood_demo_pool.json` (regenerate with `scripts/ctt_v2/derive_ood_demo_pool.py`). 🔒 **The OP SET IS UNTOUCHED — `FilmBurn_bce3e2cb2d` remains an inline-OOD op and there is NO re-draw**; replacing an op over a measured pixel property is exactly the post-filtering `A11_seven_open_items_VERBATIM.md` item 1 forbids. |
