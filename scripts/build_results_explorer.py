...
function renderAll(){
  ...
  const npass=Object.values(DATA.verdicts).filter(Boolean).length;
  $('#navchip').textContent=`overall ${DATA.overall?'PASS':'FAIL'} · ${npass}/${Object.keys(DATA.verdicts).length}`;
  $('#navchip').className='chip '+(DATA.overall?'pass':'fail');
