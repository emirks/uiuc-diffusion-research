...
# Load the feature-store library BY FILE PATH so the CLI (coverage / fsck /
# sha256sums / migrate / --help) starts instantly. Plain `import diffusion`
# runs the package __init__ (imports torch, ~25 s) — a needless 2-minute
# `--help`. feature_store.py is numpy + stdlib only (no relative imports), so it
# loads standalone. `extract` still imports the backbone registry lazily
# (diffusion.feature_extractors, inside cmd_extract) — the only torch path.
import importlib.util as _ilu  # noqa: E402


def _load_feature_store():
    path = REPO_ROOT / "src" / "diffusion" / "feature_store.py"
    spec = _ilu.spec_from_file_location("diffusion_feature_store", path)
...
MIGRATABLE = feature_store.MIGRATABLE
NAMESPACES = feature_store.NAMESPACES
NS_ARRAYS = feature_store.NS_ARRAYS
FeatureStore = feature_store.FeatureStore
legacy_filenames = feature_store.legacy_filenames
legacy_key = feature_store.legacy_key
sha256_file = feature_store.sha256_file
...
def extract_videos(store: FeatureStore, ns: str, videos, extractor, *, host, sha,
    ...
    for v in videos:
        ...
        try:
            ...
            # reuse the sha from videos/SHA256SUMS when present (no re-hash)
            store.put(v, ns, arrays, {"host": host, "code_sha": sha,
                                      "origin": "extracted"},
                      video_sha256=store.sha_from_sums(v))
            out["filled"] += 1
        except Exception as e:                           # noqa: BLE001
...
def cmd_coverage(args, store):
    ...
    cols = [_short(ns) for ns in NAMESPACES] + ["controls"]

    def _cells(cov):
        cells = [f"{cov[ns]['have']}/{cov[ns]['of']}" for ns in NAMESPACES]
        c = cov["controls"]
        cells.append(f"{c['have']} [{','.join(c['names'])}]" if c["names"]
                     else str(c["have"]))
        return cells
...
def cmd_fsck(args, store):
    ...
    for t in targets:
        ...
        ctl = f" (+{rep['n_controls']} controls)" if rep.get("n_controls") else ""
        print(f"[{status}] {t['label']}: {rep['n_features']} features{ctl} over "
