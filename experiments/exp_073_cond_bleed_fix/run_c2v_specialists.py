...
CKPT_ROOT = REPO / "outputs/training/exp_073_cond_bleed_fix"
OUT_ROOT = REPO / "outputs/videos/exp_073_cond_bleed_fix/specialists"  # /<arm> appended in main()
...
def main() -> None:
    ...
    ap.add_argument("--arm", required=True, choices=["fix", "nullA", "nullB"],
                    help="which exp_073 adapter arm to load (<cls>_<arm>) and out-root subdir")
    args = ap.parse_args()

    global OUT_ROOT
    OUT_ROOT = OUT_ROOT / args.arm   # never overwrites exp_062 originals; per-arm out-root
    ...
    if args.no_adapter:
        ...
        steps, use_suffix = [2000], bool(g["suffix_conditioning"])  # only step 2000 trained in exp_073
    ...
    ckpt_dir = CKPT_ROOT / f"{args.cls}_{args.arm}" / "checkpoints"
