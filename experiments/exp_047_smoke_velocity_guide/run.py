...
def main() -> None:
    ...
    with TeeLogger(run_dir / "run.log"):
        ...
        z1_src_run   = REPO_ROOT / cfg["z1_source"]["run_dir"]
        print(f"[info] z1 source run: {z1_src_run}")
        ...
        for idx, sample in enumerate(cfg["samples"]):
            ...
            # ── load cached z1 (skip inversion) ────────────────────────────────
            z1_path = z1_src_run / sample_id / "z1.pt"
            ...
            # ── build SMOKE TARGET (tempblend of donor real smoke + target scene) ──
            ps_ = 1
            donor_id = sample["donor"]
            dz0 = torch.load(z1_src_run / donor_id / "z0.pt", map_location="cpu",
                             weights_only=False).float().to(DEVICE)
            assert dz0.shape[1] == z0_packed.shape[1], f"donor {donor_id} grid mismatch"
            tzn = pipe._unpack_latents(z0_packed, latent_num_frames, latent_height, latent_width, ps_, ps_).float()
            dzn = pipe._unpack_latents(dz0,        latent_num_frames, latent_height, latent_width, ps_, ps_).float()
            ff = free_frames; Fm = len(ff)
            peak = float(ff.index(8)) if 8 in ff else Fm / 2.0   # darkest latent frame ~8
            sig = float(cfg.get("smoke_window_sigma", 2.0))
            ii = torch.arange(Fm, device=DEVICE, dtype=torch.float32)
            w = torch.exp(-0.5 * ((ii - peak) / sig) ** 2); w = (w / w.max())
            tsv = torch.linspace(0, 1, Fm, device=DEVICE)
            t3 = tzn[:, :, 3, :, :]; t13 = tzn[:, :, 13, :, :]
            zn_target = tzn.clone()
            for k, f in enumerate(ff):
                tgt_interp = t3 * (1 - tsv[k]) + t13 * tsv[k]            # target scene morph
                donor_mid  = dzn[:, :, f, :, :]                          # donor real smoke
                zn_target[:, :, f, :, :] = (1 - w[k]) * tgt_interp + w[k] * donor_mid
            smoke_target_packed = pipe._pack_latents(zn_target, ps_, ps_)   # [1,N,128] normalized
            smoke_guide_mask = (cmask_packed == 0).float()                  # [1,N,1], free tokens=1
            log.info("smoke target built from donor=%s  window σ=%.1f peak_idx=%.1f  free tokens=%d",
                     donor_id, sig, peak, int(smoke_guide_mask.sum().item()))
            del dz0, dzn, tzn, zn_target
            ...
            for v in variants:
                ...
                gw   = float(v.get("guide_weight", 0.0))     # smoke-target guidance weight
                sched = v.get("schedule", "const")
                # set guidance state on the inverter (read inside _x0_clamp_velocity)
                inverter.smoke_target      = smoke_target_packed if gw > 0 else None
                inverter.smoke_guide_mask  = smoke_guide_mask
                inverter.smoke_guide_weight = gw
                inverter.smoke_guide_schedule = sched
                desc = f"cfg={gs} guide_w={gw} sched={sched}"
                log.info("  variant '%s' (%s) …", name, desc)
                t_v = time.perf_counter()
                z0_regen = inverter.regenerate(z1, regen_steps, stage1_scheduler, gs, diag=None)
        ...
        log.info("")
        log.info("exp_044 regen-CFG sweep — FREE-MIDDLE PSNR (the smoke transition)")
        ...
        for s in summary:
            ...
            for n in vnames:
                val = by[n]["free_mid"]["psnr"]
                col_vals[n].append(val)
                row += f"{val:13.2f}"
            log.info(row)
        log.info("-" * len(header))
        import statistics as _st
        med = "MEDIAN".ljust(18) + "".join(f"{_st.median(col_vals[n]):13.2f}" for n in vnames)
        log.info(med)
