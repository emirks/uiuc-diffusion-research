"""exp_057 scorer — exp_053 v2 fork with a singleton-style ceiling guard.

Delta vs exp_053/run_score.py: styles with <2 reference clips (jump_transition
after dedup) get ceilings=None -> items keep RAW metrics vs the singleton
reference (the in-context imitation signal for ic_unseen quads) but no
floor/ceiling normalization; such rows are excluded from the normalized score
tables/scatter (report notes them). Config default -> eval_config.yaml.

Original exp_053 docstring:
...
CONFIG_PATH = pathlib.Path(__file__).parent / "eval_config.yaml"
...
def write_report(run_dir, all_rows, cfg, label, judge_summary_path=None):
    rows = [r for r in all_rows if r.get("norm_appearance_best") is not None]
    dropped = sorted({r["arm"] for r in all_rows if r.get("norm_appearance_best") is None})
    ...
    report = [f"# harness scores — {label} ({len(all_rows)} items, {len(rows)} normalizable)\n",
              "Normalized scores: 0 = lerp-control floor, 1 = real-clip LOO ceiling.\n"]
    if dropped:
        report.append(f"Arms with singleton-style references (raw metrics only, not in tables): {', '.join(dropped)}\n")
    report.append(score_tables(rows, trust=trust, judge_by_arm=judge_by_arm))
...
def main():
    ...
    with TeeLogger(run_dir / "run.log"):
        ...
        # strip viewer-only keys (condition_reference) that EvalItem doesn't know
        raw_manifest = json.loads(resolve(args.manifest).read_text())
        for r_ in raw_manifest:
            r_.pop("condition_reference", None)
        scoring_manifest = run_dir / "manifest_scoring.json"
        scoring_manifest.write_text(json.dumps(raw_manifest))
        items = load_manifest(scoring_manifest)
        ...
        for style, bs in ref_bundles.items():
            if len(bs) < 2:  # singleton style (jump_transition): no LOO ceiling
                print(f"[warn] style {style} has {len(bs)} ref clip(s) — no ceiling, raw metrics only")
                ceilings[style] = None
                continue
        ...
        for it in items:
            ...
            for k, hib in [("profile_dtw_best", False), ("appearance_best", True),
                ...
                row[f"floor_{k}"] = floor[k]
                if ceil is None:
                    row[f"norm_{k}"], row[f"ceil_{k}"] = None, None
                else:
                    row[f"norm_{k}"] = normalize_score(row[k], floor[k], ceil[k], higher_better=hib)
                    row[f"ceil_{k}"] = ceil[k]
        ...
        if cfg["wandb"]["enabled"]:
            try:
                import wandb
                run = wandb.init(project=cfg["wandb"]["project"], name=f"exp057_{args.label}",
                                 tags=["exp_057", "eval-harness", "ic-lora", args.label],
