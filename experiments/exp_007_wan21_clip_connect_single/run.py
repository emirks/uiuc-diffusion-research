...
One clip pair + one prompt → one video. Fast iteration loop: edit config, run.

Video is saved as:
    outputs/videos/exp_007_.../run_XXXX/{pair_id}__s{seed}__steps{N}__cfg{G}.mp4

Usage (from repo root):
    python experiments/exp_007_wan21_clip_connect_single/run.py
"""
...
def preflight_export_check() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="exp007_preflight_") as td:
...
def make_video_name(pair_id: str, seed: int, steps: int, cfg_scale: float) -> str:
    """Build a descriptive filename that encodes the key run parameters."""
    return f"{pair_id}__s{seed}__steps{steps}__cfg{cfg_scale:.1f}.mp4"
...
def main() -> None:
    ...
    video_name = make_video_name(pair_id, seed, num_steps, cfg_scale)
