"""exp_040 — LTX-2 RF-Solver inversion with comprehensive feature caching.

Fork of exp_033 (drop1 recipe — production-deployable §0 floor; see
notes/rf_inversion_postmortem.md). The inversion recipe is unchanged.

What this experiment adds. Forward hooks on the LTX-2 transformer capture
intermediate activations at configurable (step × layer × site) grids during
all three phases (invert, reconstruct, regenerate). The cache is the input
to a downstream feature-injection axis of the editing pipeline (PnP /
MasaCtrl / DiTCtrl / FreeControl / TokenFlow-style).

Captured per outer step (cheap, recorded by the loop):
  z_t_in, v_pred (raw post-CFG mix), sigma_curr / sigma_mid / sigma_next,
  dtau, t_value, phase, step_idx, substep.

Captured per (step, substep, layer) via hooks (heavy):
  block_out                        — video-stream output of transformer_blocks[l]
  attn1_q, attn1_k, attn1_v        — self-attn Q/K/V (pre-RMSNorm, pre-RoPE)
  attn2_q, attn2_k, attn2_v        — text cross-attn Q/K/V (optional)
  ff_out, audio_attn1_*, a2v_*     — further sites (optional)

See README.md and src/diffusion/feature_cache.py for the full schema,
tensor shapes, reload recipe, and caveats. The inversion recipe and
metrics are kept identical to exp_033 so any regression in PSNR/SSIM/LPIPS
between exp_033 and exp_040 is attributable to the hooks (it should be
zero — hooks are pure observers).

Workflow per sample (under run_dir/<sample_id>/):
  (a) Load source clip → VAE encode → z0  (audio: encoded silent mel)
  (b) Inversion (CFG=1)      → z1       (cache hooks active)
  (c) Reconstruction (CFG=1) → z0_recon  (cache hooks active)
  (d) Regeneration (CFG=gen) → z0_regen  (cache hooks active)
...
Saved per sample:
  ...
  feature_cache/                   — NEW: hooked tensors per phase × step
    manifest.yaml                  — schema, layer/step indices, sites, dtype
    static.pt                      — clean_latents, conditioning_mask, sigmas
    invert/step_NNN.pt             — per-step payload (one file per saved step)
    recon/step_NNN.pt
    regen/step_NNN.pt
"""
...
from diffusion.feature_cache import FeatureCache
...
class RFInverter:
    ...
    def __init__(
        ...
        *,
        ...
        self.vae = pipe.vae
        # Optional feature cache. When None, behavior matches exp_033 exactly.
        # When set, the cache's hooks must already be attached to pipe.transformer.
    ...
    def _midpoint_step(
        ...
        *,
        ...
        # Predictor substep at sigma_curr.
        if self.feature_cache is not None:
            self.feature_cache.set_substep("predictor", sigma=sigma_curr)
        v_raw = self._call_transformer(z, sigma_curr, guidance_scale=1.0)
        if self.feature_cache is not None:
            self.feature_cache.record_cheap(substep="predictor", z_in=z, v_pred=v_raw)
        v, x0_pred = self._x0_clamp_velocity(z, v_raw, sigma_curr)

        z_mid = z + (dtau / 2.0) * v

        # Corrector substep at sigma_mid.
        if self.feature_cache is not None:
            self.feature_cache.set_substep("corrector", sigma=sigma_mid)
        v_mid_raw = self._call_transformer(z_mid, sigma_mid, guidance_scale=1.0)
        if self.feature_cache is not None:
            self.feature_cache.record_cheap(substep="corrector", z_in=z_mid, v_pred=v_mid_raw)
    ...
    def _euler_step(
        ...
        *,
        ...
        if self.feature_cache is not None:
            self.feature_cache.set_substep("euler", sigma=sigma_curr)
        v_raw = self._call_transformer(z, sigma_curr, guidance_scale=guidance_scale)
        if self.feature_cache is not None:
            self.feature_cache.record_cheap(substep="euler", z_in=z, v_pred=v_raw)
        v, x0_pred = self._x0_clamp_velocity(z, v_raw, sigma_curr)
        z_next = z + dtau * v
        z_next = z_next * (1 - mask) + self.clean_latents * mask
        z_next = z_next.to(z.dtype)
    ...
    def invert(
        ...
        *,
        ...
        for i in range(len(sigmas_inv) - 1):
            ...
            sigma_mid  = sigma_curr + (sigma_next - sigma_curr) / 2.0
            if self.feature_cache is not None:
                self.feature_cache.set_step(
                    phase="invert", step_idx=i,
                    sigma_curr=sigma_curr, sigma_next=sigma_next, sigma_mid=sigma_mid,
                )
    ...
    def regenerate(
        ...
        *,
        ...
        for i in range(len(sigmas_gen) - 1):
            ...
            if self.feature_cache is not None:
                self.feature_cache.set_step(
                    phase="regen", step_idx=i,
                    sigma_curr=sigma_curr, sigma_next=sigma_next, sigma_mid=None,
                )
            z = self._euler_step(
                z, sigma_curr, sigma_next,
...
def main() -> None:
    ...
    with TeeLogger(run_dir / "run.log"):
        ...
        # ── Feature cache setup (attaches hooks to pipe.transformer) ───────────
        fc_cfg = cfg.get("feature_cache", {})
        feature_cache: FeatureCache | None = None
        if fc_cfg.get("enabled", True):
            fc_cfg = dict(fc_cfg)
            fc_cfg.setdefault("model_id", model_id)
            # Auto-expand "all" sentinels for step indices.
            for key, n in (
                ("step_indices_invert", inv_steps),
                ("step_indices_recon",  inv_steps),
                ("step_indices_regen",  regen_steps),
            ):
                v = fc_cfg.get(key, [])
                if v == "all":
                    fc_cfg[key] = list(range(n))
            # Auto-expand "all" for layer indices (must read num_blocks from pipe).
            if fc_cfg.get("layer_indices") == "all":
                fc_cfg["layer_indices"] = list(range(len(pipe.transformer.transformer_blocks)))
            feature_cache = FeatureCache(fc_cfg, log=log)
            feature_cache.attach(pipe.transformer)
            log.info(
                "Feature cache READY: %d hooks, capturing during invert/recon/regen.",
                sum(1 for _ in feature_cache._handles),
            )
        else:
            log.info("Feature cache DISABLED (feature_cache.enabled=false in config).")
        ...
        for idx, sample in enumerate(cfg["samples"]):
            ...
            # apply_visual_conditioning builds the conditioning mask geometry
            # AND the production sub-clip anchors (clean_latents from
            # separately re-encoded start/end sub-clips). exp_033 KEEPS those
            # anchors (deployable) and only drops the worst-discontinuity
            # token position from the mask (see below).
            ...
            # ── exp_033's one change vs exp_032: drop end-clip first latent frame
            # Restore production sub-clip anchors (clean_latents from
            # apply_visual_conditioning — sub-clip encoded, deployable) but
            # zero out the conditioning mask at the first latent frame of the
            # end sub-clip. That single position is maximally discontinuous
            # between sub-clip-isolated encode and full-clip encode (causal
            # VAE: fresh-start anchor in the former, full-middle context in
            # the latter). With mask=0 there, the solver fills that token
            # freely instead of being hard-pinned to a mismatched anchor.
            assert pipe.transformer_spatial_patch_size == 1 and pipe.transformer_temporal_patch_size == 1, (
                "Drop-index math assumes patch sizes 1 (LTX-2 19B). "
                "Update the indexing if running on a model with patch_size>1."
            )
            ...
            active_before = int((cmask_packed > 0).sum().item())
            cmask_packed[:, drop_start:drop_end] = 0.0
            clean_latents[:, drop_start:drop_end] = 0.0  # defensive: ignored when mask=0
            active_after = int((cmask_packed > 0).sum().item())
            log.info(
                "conditioning_mask shape=%s   active tokens=%d → %d / %d   "
                "(production sub-clip anchors; dropped end-clip first latent "
                "frame at tokens [%d:%d), end_latent_idx=%d, tokens/lframe=%d)",
                tuple(cmask_packed.shape), active_before, active_after,
                cmask_packed.shape[1], drop_start, drop_end,
                end_latent_idx, tokens_per_latent_frame,
            )

            # ── Prepare inverter ─────────────────────────────────────────────
            inverter = RFInverter(pipe, device=DEVICE, feature_cache=feature_cache)
            ...
            # ── Per-sample token scope (optional) ─────────────────────────────
            # If the config requests free-middle scoping, cache only those token
            # positions (the region we inject into). Token ids depend on this
            # sample's H'·W', so they are built here. Ascending order matches
            # the injector's fm_token_ids construction exactly.
            if feature_cache is not None:
                scope_frames = fc_cfg.get("token_scope_free_middle_frames", None)
                if scope_frames:
                    scope_ids = torch.cat([
                        torch.arange(f * tokens_per_latent_frame,
                                     (f + 1) * tokens_per_latent_frame)
                        for f in scope_frames
                    ])
                    feature_cache.set_token_scope(scope_ids)
                else:
                    feature_cache.set_token_scope(None)

            # ── Start feature cache for this sample ───────────────────────────
            if feature_cache is not None:
                feature_cache.start_sample(sample_dir)
                feature_cache.save_static({
                    "sample_id":         sample_id,
                    "prompt":            prompt,
                    "negative_prompt":   negative_prompt,
                    "clean_latents":     clean_latents.detach().cpu().to(torch.bfloat16),
                    "conditioning_mask": cmask_packed.detach().cpu().to(torch.bfloat16),
                    "z0_packed":         z0_packed.detach().cpu().to(torch.bfloat16),
                    "audio_context":     audio_context.detach().cpu().to(torch.bfloat16),
                    "latent_num_frames": latent_num_frames,
                    "latent_height":     latent_height,
                    "latent_width":      latent_width,
                    ...
                    "width":             width,
                    "frame_rate":        frame_rate,
                    "drop_token_range":  (drop_start, drop_end),
            ...
            if feature_cache is not None:
                feature_cache.end_sample()

            del attempts, attempt
            torch.cuda.empty_cache()

        if feature_cache is not None:
            feature_cache.detach()
