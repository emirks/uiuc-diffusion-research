...
Emits eval_g_abc_c{0..2}.json (3 class-chunks, like ic3_abc) + training_manifest_069.json
...
OUT_ROOT = R / "outputs/videos/exp_069_s3b"
MANIFEST = R / "experiments/exp_069_s3b/eval/manifest_069.json"
DS = R / "experiments/exp_069_s3b/eval"
...
rows = bem.ic_rows(MANIFEST, OUT_ROOT, lambda r: f"g_{r['tier'].lower()}", "s2")
parts = bem.chunk_by_class(rows, 3)
for i, part in enumerate(parts):
    (DS / f"eval_g_abc_c{i}.json").write_text(json.dumps(part, indent=1))
    print(f"[write] eval_g_abc_c{i}.json  {len(part)} rows")

tm = bem.training_manifest_ic3()
tm["adapter_id"] = "exp069_s3b_step_05000"
(DS / "training_manifest_069.json").write_text(json.dumps(tm, indent=1))
print(f"[done] {len(rows)} eval rows (arms s2_a/s2_b/s2_c) + training_manifest_069.json")
