"""exp_036 — LTX-2 RF-Solver inversion with SOFT model-bootstrap middle anchors.

Iteration It-6 of the RF-inversion research loop. Fork of exp_035 with ONE
change: the conditioning mask at middle latent frames {4..11} is set to
soft strength (default 0.3) instead of hard 1.0. The x0-clamp formula
`x0_pred * (1-mask) + clean_latents * mask` then blends 70% solver-predicted
x0 with 30% bootstrap-derived target. This gives directional guidance from
the bootstrap without forcing the round-trip through bootstrap-land (which
was exp_035's catastrophic failure mode at strength=1.0).
Forks exp_033 (drop1 at frame 12; keep sub-clip-encoded anchors at {0..3,
13..15}) with one addition: a forward C2V generation pass that produces
`z_bootstrap`, whose middle-latent slices {4..11} become deployable anchor
pins. The middle was previously free in all prior experiments; It-3's CSV
showed 60% of round-trip cost lives there, coupled to anchor quality via
velocity coupling. The bootstrap is purely derivable from {start_subclip,
end_subclip, model} — all §0-permitted inputs.

Resulting conditioning at inversion time:
  frames {0..3}    : sub-clip-encoded anchors (exp_033 baseline, unchanged)
  frames {4..11}   : NEW bootstrap-derived anchors (strength 1.0)
  frame  12        : dropped (exp_033, unchanged)
  frames {13..15}  : sub-clip-encoded anchors (exp_033 baseline, unchanged)

Bootstrap runs via `pipe(..., output_type="latent", return_dict=False)`,
which returns unpacked + denormalized latents. We re-normalize and re-pack
to obtain `z_bootstrap_packed` matching the inversion's latent format.
...
def main() -> None:
    ...
    with TeeLogger(run_dir / "run.log"):
        ...
        for idx, sample in enumerate(cfg["samples"]):
            ...
            # ── exp_035 additions: model-bootstrap middle anchors ─────────
            # Run a forward C2V generation pass with the SAME endpoint VC
            # conditions; capture z_bootstrap; use its middle slices as new
            # deployable anchors at previously-free latent frames {4..11}.
            bootstrap_cfg = cfg.get("bootstrap", {})
            bootstrap_steps = int(bootstrap_cfg.get("num_steps", 20))
            bootstrap_cfg_scale = float(bootstrap_cfg.get("guidance_scale", 4.0))
            bootstrap_decode_video = bool(bootstrap_cfg.get("decode_video", False))
            middle_first = (num_clip_frames - 1) // LTX_TEMPORAL_SCALE + 1  # = 4
            # middle goes up to (but not including) end_latent_idx.

            log.info(
                "[bootstrap] running C2V generation: %d steps, CFG=%.1f, "
                "endpoint conditions identical to inversion.",
                bootstrap_steps, bootstrap_cfg_scale,
            )
            t_boot = time.perf_counter()
            # Fresh generator for the bootstrap so we don't perturb the
            # deterministic invert generator state. Seed offset is documented.
            boot_generator = torch.Generator(device=DEVICE).manual_seed(seed + 1)
            bootstrap_out = pipe(
                ...
                num_inference_steps=bootstrap_steps,
                guidance_scale=bootstrap_cfg_scale,
                output_type="latent",
                return_dict=False,
                generator=boot_generator,
            )
            # pipe(...) with output_type="latent", return_dict=False returns
            # (video_latents_5d_unpacked_denorm, audio_latents_5d_unpacked_denorm)
            z_bootstrap_5d_denorm = bootstrap_out[0]
            z_bootstrap_packed = normalize_and_pack(pipe, z_bootstrap_5d_denorm).to(
                device=DEVICE, dtype=z0_packed.dtype
            )
            t_boot_done = time.perf_counter() - t_boot
            log.info(
                "[bootstrap] done in %.1fs.  z_bootstrap_packed shape=%s  norm=%.2f",
                t_boot_done, tuple(z_bootstrap_packed.shape),
                z_bootstrap_packed.float().norm().item(),
            )
            if bootstrap_decode_video:
                t_dec = time.perf_counter()
                bootstrap_video = decode_latents_to_video(pipe, z_bootstrap_5d_denorm)
                save_video(
                    sample_dir / "bootstrap_video.mp4", bootstrap_video, fps=int(frame_rate)
                )
                log.info(
                    "[bootstrap] decoded mp4 for smoke check in %.1fs.",
                    time.perf_counter() - t_dec,
                )

            middle_strength = float(bootstrap_cfg.get("middle_strength", 0.3))
            middle_start = middle_first * tokens_per_latent_frame
            middle_end   = end_latent_idx * tokens_per_latent_frame
            clean_latents[:, middle_start:middle_end, :] = z_bootstrap_packed[
                :, middle_start:middle_end, :
            ]
            cmask_packed[:, middle_start:middle_end] = middle_strength
            log.info(
                "[exp_036 soft-bootstrap] middle pin strength=%.2f (was 1.0 in exp_035)",
                middle_strength,
            )
            ...
            log.info(
                ...
                "(exp_035: sub-clip anchors at {0..%d, %d..%d} kept, frame %d "
                "dropped, bootstrap anchors NEW at {%d..%d} tokens [%d:%d).)",
                tuple(cmask_packed.shape), active_before, active_after,
                cmask_packed.shape[1],
                middle_first - 1, end_latent_idx + 1, latent_num_frames - 1,
                end_latent_idx,
                middle_first, end_latent_idx - 1,
                middle_start, middle_end,
            )
            ...
            torch.save(
                z_bootstrap_packed.detach().cpu().to(torch.bfloat16),
                sample_dir / "z_bootstrap.pt",
            )
            ...
            meta = {
                ...
                "bootstrap": {
                    "num_steps":      bootstrap_steps,
                    "guidance_scale": bootstrap_cfg_scale,
                    "wall_s":         round(t_boot_done, 1),
                    "z_bootstrap_norm": float(z_bootstrap_packed.float().norm().item()),
                    "middle_token_range": [middle_start, middle_end],
                },
        ...
        log.info("")
        log.info("exp_035 RF inversion (bootstrap middle) — metrics summary  (gate is informational; no retry)")
        log.info(
