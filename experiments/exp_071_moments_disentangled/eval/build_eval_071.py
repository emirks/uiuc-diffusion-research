...
Emits eval_m_abc_c{0..2}.json (3 class-chunks, like ic3_abc) + training_manifest_071.json
...
OUT_ROOT = R / "outputs/videos/exp_071_moments_disentangled"
MANIFEST = R / "experiments/exp_071_moments_disentangled/eval/manifest_071.json"
DS = R / "experiments/exp_071_moments_disentangled/eval"
...
rows = bem.ic_rows(MANIFEST, OUT_ROOT, lambda r: f"m_{r['tier'].lower()}", "m")
parts = bem.chunk_by_class(rows, 3)
for i, part in enumerate(parts):
    (DS / f"eval_m_abc_c{i}.json").write_text(json.dumps(part, indent=1))
    print(f"[write] eval_m_abc_c{i}.json  {len(part)} rows")

tm = bem.training_manifest_ic3()
tm["adapter_id"] = "exp071_moments_step_05000"
(DS / "training_manifest_071.json").write_text(json.dumps(tm, indent=1))
print(f"[done] {len(rows)} eval rows (arms s2_a/s2_b/s2_c) + training_manifest_071.json")
