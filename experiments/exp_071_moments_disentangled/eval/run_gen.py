...
OUT_ROOT = REPO / "outputs/videos/exp_071_moments_disentangled"
GAMMA = 1.0922
...
def main() -> None:
    ...
    print(f"[info] moments-mode gen {len(todo)}/{len(rows)} (gamma={GAMMA})")
    if not todo:
    ...
    runner = ValidationRunner(config=val_cfg, model_path=MODEL, text_encoder_path=GEMMA,
                              residual_reference=True, residual_gamma=GAMMA, residual_mode='moments')
