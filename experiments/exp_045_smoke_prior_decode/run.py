    ...
    with TeeLogger(run_dir / "run.log"):
        ...
        for idx, s in enumerate(cfg["samples"]):
            ...
            for name in priors:
                ...
                fm_ssim = metric_suite.ssim(ref_video[px_lo:px_hi], vid[px_lo:px_hi])["mean"]
                full = metric_suite.psnr(ref_video, vid)["mean"]
                log.info("  [%-26s] FREE-MID PSNR=%6.2f  ssim=%.3f  full=%6.2f", name, fm, fm_ssim, full)
                results.append({"name": name, "free_mid_psnr": round(float(fm), 2),
                                "free_mid_ssim": round(float(fm_ssim), 3), "full_psnr": round(float(full), 2)})
                del zn_mod, zd, vid, prior
                torch.cuda.empty_cache()

            summary.append({"sample_id": sid, "latent_HxW": [lh, lw],
                            "free_mid_pixel_range": [px_lo, px_hi], "priors": results})
