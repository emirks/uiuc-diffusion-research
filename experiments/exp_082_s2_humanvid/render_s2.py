"""ctt_v2 S2b renderer (UNION bank) — exact ops x content pairs over two REAL PLAYING streams.

exp_082 = exp_081_s2_stratum re-pointed at the merged union of the synth and HumanVid endpoint
banks. Diffs vs the exp_081 original are path/name only: it reads PLAN_S2_UNION.json, writes under
outputs/videos/ctt_v2_s2_humanvid/, takes its shader policy from this directory's local
D2_POLICY_V2.json (easings from the local D2_POLICY_FINAL.json, which V2 does not carry), and
imports exp_077's helper modules from the MAIN tree because that experiment is not checked out
on this branch. The grid sizes are NOT settled — see the TODOs in config_s2.yaml.
...
# exp_077_synth_stratum (d2_metrics.py, streams_real.py) is not checked out on the ctt-v2
...
def main() -> None:
    ...
    # D2_POLICY_V2.json is a SHADER-only revision and carries no easing list; the easing set
    # comes from the frozen v1 policy via inputs.policy_easings (identical easings either way).
    if "keep_easings" not in policy:
        policy = json.loads((REPO_ROOT / cfg["inputs"]["policy_easings"]).read_text())
