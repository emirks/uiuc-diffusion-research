...
class MeshRenderer:
    ...
    @staticmethod
    def _egl_context():
        """Create a headless EGL context, falling back across EGL devices.

        exp_083: on a GPU node requested WITHOUT --gres, glcontext's EGL backend
        picks device 0 via EGL_EXT_platform_device — an NVIDIA card this job has
        no permission to open — and eglInitialize returns 0x3001. Forcing
        LIBGL_ALWAYS_SOFTWARE does not help ("not allowed to force software
        rendering when API explicitly selects a hardware device"). Mesa also
        exposes a swrast EGL device, so the fix is to walk the device list until
        one initialises. Costs nothing when device 0 already works (login nodes,
        CPU nodes) because that is tried first.
        """
        import os
        last = None
        for idx in [None] + list(range(16)):
            if idx is None:
                os.environ.pop("GLCONTEXT_DEVICE_INDEX", None)
            else:
                os.environ["GLCONTEXT_DEVICE_INDEX"] = str(idx)
            try:
                return moderngl.create_standalone_context(backend="egl"), idx
    ...
    def __init__(self, width: int, height: int, step: int = 2):
        ...
        self.ctx, self.egl_device_index = self._egl_context()
    ...
    def renderer_name(self) -> str:
        return (f"{self.ctx.info['GL_RENDERER']} | {self.ctx.info['GL_VERSION']}"
                f" | egl_device_index={self.egl_device_index}")
