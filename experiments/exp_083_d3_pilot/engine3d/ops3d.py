...
def subject_anchor(view_z: np.ndarray, rgb: np.ndarray) -> tuple[int, int]:
    """Pixel (row, col) of the foreground subject: the saliency-weighted centroid
    of the nearest quartile of the depth map.

    exp_083 addition. The D2 post-mortem said the losses concentrate on
    *content-coupled* donors — shadow/smoke effects that attach to a foreground
    object and travel with it. A screen-space shader cannot do that; a world-space
    field anchored on the subject can, but only if it knows where the subject is.
    Depth answers that for free (the subject is the near blob) and spectral-residual
    saliency breaks the tie between several near blobs. No external metadata,
    so it works on any endpoint.
    """
    near = view_z <= np.percentile(view_z, 25.0)
    w = _saliency_sr(rgb) * near
    if w.sum() < 1e-6:
        w = near.astype(np.float32)
    ys, xs = np.mgrid[0:view_z.shape[0], 0:view_z.shape[1]]
    tot = float(w.sum()) + 1e-8
    return int(round(float((ys * w).sum()) / tot)), int(round(float((xs * w).sum()) / tot))


def dissolve_field(kind: str, P: np.ndarray, freq: float, seed: int,
    ...
    `center` (exp_083) pins the `sphere`/`subject` shell to a given scene point —
    the foreground subject's world position — so the effect emanates from the
    object rather than from an arbitrary place in the volume.
    """
    rng = np.random.default_rng(seed)
    if kind == "plane":                      # a real plane sweeping through the scene
        n = rng.normal(size=3); n /= np.linalg.norm(n)
        f = P @ n.astype(np.float32)
    elif kind in ("sphere", "subject"):      # an expanding shell about a scene point
        if kind == "subject" and center is not None:
            c = np.asarray(center, dtype=np.float32)
        else:
            c = np.array([rng.uniform(-1, 1), rng.uniform(-1, 1),
                          -rng.uniform(1.2, 3.0)], dtype=np.float32)
        f = np.linalg.norm(P - c, axis=-1)
    elif kind == "subject_fbm":
        # fBm noise MODULATED by distance from the subject: the erosion front is
        # noisy (so it reads as smoke/decay rather than a geometric shell) but it
        # still starts at the subject and travels outward with it.
        base = dissolve_field("fbm", P, freq, seed)
        c = (np.asarray(center, dtype=np.float32) if center is not None
             else np.array([0.0, 0.0, -2.0], dtype=np.float32))
...
def _coverage_stats(den: np.ndarray, k: int) -> dict:
    """Disocclusion audit for one composited frame.

    `den` is the total alpha available at each pixel. Where it is ~0 the camera
    move revealed geometry neither layer has, and `_fill_holes` invents the colour.
    HOW MUCH is missing is not the signal (a healthy clip runs ~10% and the
    push-pull repair is invisible); what matters is how FAR a hole pixel is from
    any real one, because the cascade blurs at 9/31/81 px and cannot pull colour
    across more than ~40 px. `hole_radius` is that distance, measured with a
    distance transform — the same quantity exp_082 calibrated on the stream driver.
    """
    hole = (den < COVER_HARD).astype(np.uint8)
    weak = (den < COVER_WEAK).astype(np.uint8)
    r = (float(cv2.distanceTransform(hole, cv2.DIST_L2, 5).max())
         if hole.any() else 0.0)
    rw = (float(cv2.distanceTransform(weak, cv2.DIST_L2, 5).max())
          if weak.any() else 0.0)
    return {"k": int(k), "uncovered": float(hole.mean()), "weak": float(weak.mean()),
            "hole_radius": r, "weak_radius": rw}


def render_transition(renderer, op: Operator3D, start9: np.ndarray,
                      end9: np.ndarray, disp_a: np.ndarray, disp_b: np.ndarray,
                      n_middle: int, *, coverage_out: list | None = None
                      ) -> np.ndarray:
    ...
    `coverage_out` (exp_083) collects the per-frame disocclusion audit — how much
    of the frame no mesh covered and how far the largest hole is from any real
    pixel. Same signal exp_082 added to the stream driver; costs nothing because
    `den` already exists inside `composite`.
    """
    ...
    # For the subject-anchored kinds the shell is centred on the foreground
    # object's own unprojected position, so the effect emanates from the subject
    # and rides with it under parallax — the content coupling D2 could not express.
    fld_a = fld_b = None
    if op.dissolve != "none":
        Pa, Pb = world_positions(za, op.fovy), world_positions(zb, op.fovy)
        ca = cb = None
        if op.dissolve in ("subject", "subject_fbm"):
            ra, cca = subject_anchor(za, frame_a)
            rb, ccb = subject_anchor(zb, frame_b)
            ca, cb = Pa[ra, cca], Pb[rb, ccb]
        fld_a = dissolve_field(op.dissolve, Pa, op.dissolve_freq, op.seed, ca)
        fld_b = dissolve_field(op.dissolve, Pb, op.dissolve_freq, op.seed + 1, cb)
    ...
    for k in range(n_middle):
        ...
        for j in range(mb):
            ...
            if coverage_out is not None and j == 0:
                f_u8, den = composite(ra, rb, blend_weight(op, eb, disp_b),
                                      return_den=True)
                coverage_out.append(_coverage_stats(den, k))
...
# --------------------------------------------------------------------------
# exp_080: the REAL-STREAM driver — both layers keep PLAYING (per-frame texture
# + per-frame depth); the camera transition occupies a D2-style window inside a
# fixed 121-frame clip. Pure phases are source frames verbatim, so endpoint
# fidelity is byte-exact by construction, like D2 — while the ramp adds the one
# manner family 2D shaders cannot fake (parallax, dolly-zoom, world-space
# dissolves through live scenes).
# --------------------------------------------------------------------------
...
def render_transition_stream(renderer, op: Operator3D, A: np.ndarray,
    ...
    ease = EASINGS[op.easing]
    bease = EASINGS[op.blend_easing]
    jitter = (cameras.handheld(op.seed, n_mid, op.handheld)
              if op.handheld > 0 else None)

    # Dolly-zoom subject depth: held from the first ramp frame (a per-frame
    # median would flicker the focal length).
    za0 = to_view_depth(da_stack[onset + 1], op.depth_near, op.depth_far,
                        op.depth_gamma)
    h, w = za0.shape
    z_subj = float(np.median(za0[h // 3: 2 * h // 3, w // 3: 2 * w // 3]))

    mb = max(1, op.motion_blur)
    du = 1.0 / (n_mid + 1)

    def layer(frame, z, disp, s, e, eb, jit, is_a):
        v, sh = path(s, pivot=op.pivot, axis=op.axis, turns=op.turns)
        if jit is not None:
            v = cameras.apply_handheld(v, jit)
        fov = op.fovy
        ...
        if op.dissolve != "none":
            # World-space field re-evaluated at the CURRENT frame's positions:
            # the pattern sticks to (moving) surfaces instead of the screen.
            fld = dissolve_field(op.dissolve, world_positions(z, op.fovy),
                                 op.dissolve_freq, op.seed + (0 if is_a else 1))
            ed = op.dissolve_edge
            tau = -ed + (eb if is_a else 1.0 - eb) * (1.0 + 2 * ed)
            m = np.clip((fld - tau) / (2 * ed) + 0.5, 0.0, 1.0)
            f = np.concatenate([f, (m * 255).astype(np.uint8)[..., None]], axis=2)
    ...
    out = np.empty_like(A)
    out[: onset + 1] = A[: onset + 1]
    out[release:] = B[release:]

    for k in range(n_mid):
        t = onset + 1 + k
        u = (k + 1) * du                      # camera clock, open interval
        vb_ = k / max(n_mid - 1, 1)           # blend clock, closes inside
        za = to_view_depth(da_stack[t], op.depth_near, op.depth_far, op.depth_gamma)
        zb = to_view_depth(db_stack[t], op.depth_near, op.depth_far, op.depth_gamma)
        acc = None
        for j in range(mb):
            uj = u + (j / mb - 0.5) * du * 0.5
            e = float(ease(np.clip(np.float64(uj), 0.0, 1.0)))
...
def join_ratio(clip: np.ndarray, joint: int, side: int = 4) -> float:
    """Frame delta ACROSS a join relative to the local internal motion (1.0 =
    the join moves exactly as much as the content around it)."""
    d = np.abs(np.diff(clip[max(0, joint - side): joint + side + 1]
                       .astype(np.float32), axis=0)).mean(axis=(1, 2, 3))
    at = d[side - 1] if side - 1 < len(d) else d[-1]
    rest = np.delete(d, side - 1) if len(d) > 1 else d
    return float(at / max(float(rest.mean()), 1e-3))
