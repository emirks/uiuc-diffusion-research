...
Iteration It-4 of the RF-inversion research loop (notes/rf_inversion_loop.md).
Forks exp_033 (which dropped one end-sub-clip latent frame) with a
recipe-switched conditioning block. It-3's CPU diagnostic
(scripts/anchor_error_localization.py) measured |z0 - z0_recon|² per latent
frame across exp_030/032/033 and showed:
  • 60% of exp_030's total round-trip cost lives in free middle frames;
  • the largest single-position anchor mismatch is end-sub-clip frame 12,
    driven by causal-VAE's 1-pixel vs 8-pixel collapse asymmetry;
  • exp_033's drop1 zeroed frame-12 cost (98% local reduction) but left
    start-side mismatch (57K cumulative) and middle truncation in place.

This experiment tests two deployable interventions under §0.

- recipe=A "scaffold_pad":
    Build an 8-frame static-replay clip [end_frames[0]] × 8, VAE-encode,
    and substitute the resulting single latent frame as the anchor at
    source latent frame 12 (re-enabling cmask there). Replaces "no pin"
    (exp_033's drop) with "8-pixel-collapse pin from deployable inputs".

- recipe=B "drop_all_end":
    Zero cmask + clean_latents at all four end-sub-clip latent positions
    {12, 13, 14, 15}; keep only the start anchors {0..3}. Tests whether
    cumulative end-side anchor mismatch is net-harmful.

Both recipes use only the two endpoint sub-clips — deployable per §0
(deleting middle source frames leaves both recipes' inputs intact).

Everything else identical to exp_033: per-clip `max_area` resolution,
...
def main() -> None:
    ...
    with TeeLogger(run_dir / "run.log"):
        ...
        for idx, sample in enumerate(cfg["samples"]):
            ...
            # ── exp_034 recipe switch ─────────────────────────────────────
            # Both recipes start from exp_033's baseline (apply_visual_conditioning
            # has produced sub-clip anchors at start{0..3} and end{12..15}).
            assert pipe.transformer_spatial_patch_size == 1 and pipe.transformer_temporal_patch_size == 1, (
                "Index math assumes patch sizes 1 (LTX-2 19B)."
            )
            tokens_per_latent_frame = latent_height * latent_width
            end_latent_idx = max(int(i) for i in condition_indices)
            recipe = str(inv_cfg.get("recipe", "A")).upper()
            active_before = int((cmask_packed > 0).sum().item())

            if recipe == "A":
                # Scaffold-pad: build [end_frames[0]] × 9 (the minimum frame
                # count whose VAE encoding has ≥ 2 latent frames AND whose
                # latent frame 1 collapses 8 pixel frames — i.e. matches the
                # full-clip encoder's 8-pixel-collapse semantics at the
                # source-latent-12 position). LTX-2 VAE requires
                # (F_pix - 1) % 8 == 0 for clean temporal downsampling; 9 is
                # the smallest such count > 1.
                #
                # F_lat for 9 pixel frames = (9-1)//8 + 1 = 2. Latent frame 0
                # collapses pixel[0] alone (1-pixel-collapse asymmetry — the
                # SAME asymmetry that ruined exp_030's frame-12 anchor; we do
                # NOT use this). Latent frame 1 collapses pixels [1..8] (=
                # 8 copies of end_frames[0]) — this is the 8-pixel-collapse
                # latent we substitute at source latent frame 12.
                static_frames = [end_frames[0]] * 9
                static_tensor = pipe.video_processor.preprocess_video(
                    static_frames, height=height, width=width
                ).to(device=DEVICE, dtype=pipe.vae.dtype)
                static_5d = retrieve_latents(
                    pipe.vae.encode(static_tensor), generator=generator, sample_mode="argmax"
                )
                static_5d = pipe._normalize_latents(
                    static_5d, pipe.vae.latents_mean, pipe.vae.latents_std
                ).to(device=DEVICE, dtype=z0_packed.dtype)
                static_packed = pipe._pack_latents(
                    static_5d,
                    pipe.transformer_spatial_patch_size,
                    pipe.transformer_temporal_patch_size,
                )  # (1, 2 * tokens_per_latent_frame, 128) — 2 latent frames
                # Slice the second packed latent frame (index 1), which is
                # the 8-pixel-collapse latent.
                static_lframe1 = static_packed[
                    :, tokens_per_latent_frame : 2 * tokens_per_latent_frame, :
                ]
                slot_start = end_latent_idx * tokens_per_latent_frame
                slot_end   = slot_start + tokens_per_latent_frame
                clean_latents[:, slot_start:slot_end, :] = static_lframe1
                cmask_packed[:, slot_start:slot_end]     = 1.0
                active_after = int((cmask_packed > 0).sum().item())
                log.info(
                    "[recipe=A scaffold_pad] static-replay 9-frame encode (2 lframes) → "
                    "slice lframe[1] (8-pixel-collapse) as anchor at end_latent_idx=%d "
                    "tokens [%d:%d). static_packed shape=%s  active tokens=%d → %d / %d.",
                    end_latent_idx, slot_start, slot_end,
                    tuple(static_packed.shape),
                    active_before, active_after, cmask_packed.shape[1],
                )

            elif recipe == "B":
                # Drop-all-end: zero cmask + clean_latents at all 4 end-sub-clip
                # latent positions {end_latent_idx .. end_latent_idx+3} =
                # {12, 13, 14, 15} for num_clip_frames=25, num_frames=121.
                k_lat = (num_clip_frames - 1) // LTX_TEMPORAL_SCALE + 1  # = 4
                drop_start = end_latent_idx * tokens_per_latent_frame
                drop_end   = (end_latent_idx + k_lat) * tokens_per_latent_frame
                cmask_packed[:, drop_start:drop_end] = 0.0
                clean_latents[:, drop_start:drop_end] = 0.0
                active_after = int((cmask_packed > 0).sum().item())
                log.info(
                    "[recipe=B drop_all_end] zeroed %d latent frames at end-sub-clip "
                    "(positions %d..%d, tokens [%d:%d)). active tokens=%d → %d / %d.",
                    k_lat, end_latent_idx, end_latent_idx + k_lat - 1,
                    drop_start, drop_end,
                    active_before, active_after, cmask_packed.shape[1],
                )

            else:
                raise ValueError(f"unknown recipe {recipe!r}; expected 'A' or 'B'")
        ...
        log.info("")
        log.info("exp_034 RF inversion (recipe=%s) — metrics summary  (gate is informational; no retry)", recipe)
        log.info(
