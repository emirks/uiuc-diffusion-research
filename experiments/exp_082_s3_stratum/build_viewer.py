...
SECTIONS = [
    ("effect_", "Optical-effect showcase — one physically-motivated axis at a time",
     "Dolly-zoom, depth wipe, world-space dissolve, fog, rack focus, handheld+motion-blur — "
     "each isolated on a dolly base so its contribution is visible on its own, over LIVE streams."),
...
def card(m: dict, rel: str = ".") -> str:
    r = max(m["join_ratio_in"], m["join_ratio_out"])
...
<div class="meta">
  ...
  <div class="kv">{html.escape(m['pair_id'])} · window {m['onset']}–{m['release']}</div>
  <div class="kv">join ratio <span class="{cls}">{r:.2f}</span></div>
</div></div>"""
...
def main() -> None:
    ...
    ratios = [max(m["join_ratio_in"], m["join_ratio_out"]) for m in man]
    ratios.sort()
    med = ratios[len(ratios) // 2]
    fams = len({m["family"] for m in man})
    mean_s = sum(m["render_s"] for m in man) / len(man)

    parts = [f"""<title>exp_080 — 3D transitions, 121 frames, real playing streams</title>
<style>{CSS}</style><script>{JS}</script>
<div class="wrap">
<h1>3D-plausible procedural transitions</h1>
<p class="sub">Each clip is <b>121 frames</b> over two REAL clips PLAYING in lockstep: frames outside the transition window are source frames <b>verbatim</b> (pure phases byte-identical, endpoint blocks included). Inside the window a single continuous virtual camera flies out of scene A and comes to rest in scene B — but every rendered frame unprojects the <b>current</b> frame of each live stream through its own temporally-stabilised Depth&nbsp;Anything&nbsp;V2 depth map, so the world never freezes. Parallax is real; fog is Beer–Lambert; defocus is circle-of-confusion; dissolves are evaluated at world positions and stick to moving surfaces.</p>
<div class="stats">
<div class="stat"><b>{len(man)}</b><span>clips</span></div>
<div class="stat"><b>{fams}</b><span>camera families</span></div>
<div class="stat"><b>{med:.2f}</b><span>median join ratio (1.0 = as smooth as the content's own motion)</span></div>
<div class="stat"><b>{mean_s:.1f}s</b><span>mean render per clip</span></div>
</div>
<div class="toggle">
