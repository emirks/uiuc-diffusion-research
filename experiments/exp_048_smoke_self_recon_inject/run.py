...
def main() -> None:
    ...
    with TeeLogger(run_dir / "run.log"):
        ...
        for idx, sample in enumerate(cfg["samples"]):
            ...
            # ── build GUIDANCE TARGET = the clip's OWN cached recon (z0_recon) ──
            # exp_044 showed z1 ENCODES the middle (recon midpoint/CFG=1 reaches
            # 27-38 dB free-mid for z1-rich clips) but the production Euler sampler
            # loses it (solver mismatch). Here we velocity-inject the clip's own
            # deployable reconstruction into the production regen free-middle so the
            # editable production path reproduces the faithful inverted middle →
            # regen PSNR toward recon's ceiling (>18). z0_recon is a function of
            # {z1, endpoints, model} — deployable, no z0 leak.
            ps_ = 1
            z0_recon = torch.load(z1_src_run / sample_id / "z0_recon.pt", map_location="cpu",
                                  weights_only=False).float().to(DEVICE)
            assert z0_recon.shape[1] == z0_packed.shape[1], "z0_recon grid mismatch"
            smoke_target_packed = z0_recon.clone()                          # [1,N,128] normalized packed
            smoke_guide_mask = (cmask_packed == 0).float()                  # [1,N,1], free tokens=1
            log.info("guidance target = own z0_recon  free tokens=%d  ‖recon‖=%.2f",
                     int(smoke_guide_mask.sum().item()), z0_recon.float().norm().item())
            del z0_recon
