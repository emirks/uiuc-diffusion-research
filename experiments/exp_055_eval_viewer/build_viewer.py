...
Emits under <out>/:
  ...
  index.html           — copied verbatim from viewer_template.html
...
def feats_for(path: pathlib.Path, cache_dir: pathlib.Path, legacy_paths=()):
    ...
    legacy_paths: earlier locations of the same file. The cache key hashes
    (abspath, mtime, size); a plain `mv` preserves mtime+size, so a moved
    clip's features are recoverable by hashing its OLD path with its CURRENT
    stat (needed after the 2026-07-08 corpus reorg)."""
    p = pathlib.Path(path)
    try:
        st = p.stat()
    except FileNotFoundError:
        return None
    for cand in (str(p.resolve()), *(str(lp) for lp in legacy_paths)):
        raw = "|".join([cand, str(st.st_mtime_ns), str(st.st_size), DINO_MODEL, SHORT_SIDE])
        k = hashlib.sha1(raw.encode()).hexdigest()[:16]
        for cache in (cache_dir / f"dino_arr_{hashlib.sha1(k.encode()).hexdigest()[:16]}.npz",
                      cache_dir / f"dino_{k}.npz"):
            if cache.exists():
...
def main():
    ...
    ap.add_argument("--template", default=str(pathlib.Path(__file__).parent / "viewer_template.html"))
    ...
    # Layout-agnostic: clips may live flat (<style>/*.mp4) or nested inside
    # container dirs (onesided_transitions/<class>/*.mp4 after the 2026-07-08
    # corpus reorg). Scan every leaf dir that directly holds mp4s, and build a
    # filename-stem index so exam clips resolve wherever they moved to.
    stem_index = {}
    for p in sorted(troot.rglob("*.mp4")):
        active = "_dup" not in p.parts
        cur = stem_index.get(p.stem)
        if cur is None or (active and "_dup" in cur.parts):
            stem_index[p.stem] = p
    styles = []
    leaf_dirs = sorted({p.parent for p in troot.rglob("*.mp4") if "_dup" not in p.parts})
    for d in leaf_dirs:
        if d.name in args.exclude or any(part in args.exclude for part in d.parts):
            continue
    ...
    # ref_counts from the exam's own clip list (not the on-disk dir names,
    # which may have been renamed since the run) — faithful to the run viewed.
    val_results = json.loads((resolve(args.validation) / "results.json").read_text())
    ref_counts = {}
    for name in exam["names"]:
        st = name.split("/", 1)[0]
        ref_counts[st] = ref_counts.get(st, 0) + 1
    ...
    def clip_video(name):
        ...
        for c in (troot / style / f"{clip}.mp4", troot / style / "_dup" / f"{clip}.mp4"):
            if c.exists():
                return c
        return stem_index.get(clip)   # corpus reorganized since the run: find by filename
    ...
    for name in exam["names"]:
        ...
        if vp is not None:
            ...
            style, clip = name.split("/", 1)
            legacy = (troot / style / f"{clip}.mp4",          # pre-reorg flat layout
                      troot / style / "_dup" / f"{clip}.mp4")
            fe = feats_for(vp, cache_dir, legacy_paths=legacy)
            if fe is not None:
    ...
    for row in raw_items:
        iid = row["item_id"]
        m = manifest.get(iid, {})
        vids = {"generated": None, "prefix": None, "suffix": None}
