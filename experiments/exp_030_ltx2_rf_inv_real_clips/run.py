...
Fork of exp_029. We skip Stage-1 generation entirely: z0 comes from
VAE-encoding an existing source video rather than from a `pipe(...)`
call. VC conditioning is the SAME video's first 1s + last 1s.

Differences vs exp_029:
  • No Stage-1 gen, no audio capture. Audio context for invert/recon/regen
    is synthesized zeros, with shape derived from `pipe.audio_*` config
    (audio_sampling_rate / audio_hop_length / audio_vae_temporal_compression_ratio,
    mel_bins, latent channels).
  • One source clip per sample (single `clip:` path) — both VC endpoints
    are slices of it (first and last `num_clip_frames` frames).
  • Resolution auto-resolves per clip via `max_area` + first-frame aspect.

Workflow per sample (under run_dir/<sample_id>/):
  (a) Load source clip → VAE encode → z0  (audio: synthesized zeros)
  (b) Inversion (CFG=1)      → z1       (audio: zeros)
  (c) Reconstruction (CFG=1) → z0_recon (audio: zeros, matches inv)
  (d) Regeneration (CFG=gen) → z0_regen (audio: zeros, matches inv+recon)
