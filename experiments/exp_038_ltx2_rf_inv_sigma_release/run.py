"""exp_038 — LTX-2 RF-Solver inversion with σ-conditional anchor release.

Iteration It-8 of the RF-inversion research loop. Forks exp_033 (drop1 frame 12,
sub-clip anchors at {0..3, 13..15}) with ONE addition: anchors are RELEASED
(set to 0) when σ < sigma_release_threshold. Motivated by It-7's finding that
step escalation hurts passing clips (more time at low σ → wrong-anchor drift
amplifies). σ-conditional release lets the solver be FREE at low σ (close to
data, where wrong-anchor pins amplify drift) while keeping anchors at high σ
(early in trajectory, where they help with navigation).
...
class RFInverter:
    ...
    def __init__(self, pipe: LTX2ConditionPipeline, device: str = DEVICE) -> None:
        ...
        # σ-conditional anchor release: anchors are active only when
        # σ >= sigma_release_threshold. Below threshold, _eff_mask returns
        # zero so the solver is fully free during the low-σ settling phase
        # where wrong-anchor pins amplify drift (exp_037 evidence).
        self.sigma_release_threshold: float = 0.0  # 0.0 = no release (legacy)
    ...
    def prepare_sample(
        ...
        *,
        ...
        # sigma_release_threshold set externally before solver calls; default 0.
    ...
    def _eff_mask(self, sigma_scalar: float) -> torch.Tensor:
        """Effective conditioning mask at this σ. Anchors are released
        (mask returns 0) when σ < sigma_release_threshold; otherwise the
        full conditioning_mask is used. Released anchors let the solver
        be fully free at low σ where wrong-anchor pins amplify drift.
        """
        if (
            self.sigma_release_threshold > 0.0
    ...
    def _call_transformer(
        ...
        *,
        ...
        # σ-conditional release: at low σ the effective mask is zero so all
        # tokens see the full diffusion time t (no per-token override).
        ...
        cond_mask_t = self._eff_mask(sigma_scalar).squeeze(-1).to(t_dtype)
        if bsz_call == 2:
    ...
    def _x0_clamp_velocity(
        ...
        mask = self._eff_mask(sigma_scalar)
    ...
    def _midpoint_step(
        ...
        *,
        ...
        # σ-conditional: use effective mask at sigma_next for the post-step
        # re-clamp (this is where z_next is committed at the new σ level).
        mask = self._eff_mask(sigma_next)
...
def main() -> None:
    ...
    with TeeLogger(run_dir / "run.log"):
        ...
        for idx, sample in enumerate(cfg["samples"]):
            ...
            sigma_release_threshold = float(
                inv_cfg.get("sigma_release_threshold", 0.0)
            )
            inverter.sigma_release_threshold = sigma_release_threshold
            log.info(
                "[exp_038] sigma_release_threshold=%.3f (anchors release when σ<this)",
                sigma_release_threshold,
            )
