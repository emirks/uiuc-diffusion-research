"""exp_063 — R4/R5 generation via the trainer's ValidationRunner (exp_058 fork).

Reads dataset/ladder_r4r5.json (rows: reference + endpoints + prefix_only + prompt).
Generalist adapter = exp_058 ic2 step_05000, run in its NATIVE keyed mode: suffix
condition included iff the row is two_sided (prefix_only=False). Recipe pinned to the
exp_058/061 contract (480x640x121@24, 30 steps, CFG 4, STG 1 stg_v[29]).

Run in the ltx-trainer uv env:
    cd $LAB/LTX-2-official/packages/ltx-trainer
    uv run --frozen python <this file> --seed 42 [--chunk i --num-chunks n] [--include-deferred]

Skip-if-exists on <rung>/<id>__s<seed>.mp4 (requeue-safe). hero_flight R5 rows are
deferred (sidedness pending) — excluded unless --include-deferred.
"""
...
EXP = REPO / "experiments/exp_063_ladder_r4r5_generalist"
STD = REPO / "data/processed/transitions_std121"
OUT_ROOT = REPO / "outputs/videos/exp_063_ladder_r4r5_generalist"

# ic2 = attn + FFN targets (exp_058)
TARGET_MODULES = [
...
def main() -> None:
    ...
    if not args.include_deferred:
        rows = [r for r in rows if not r["deferred"]]
    ...
    print(f"[info] seed={args.seed} chunk={args.chunk}/{args.num_chunks}: "
    ...
    step = 0
    m = re.search(r"step_(\d+)", adapter.name)
    ...
    print("[info] loading transformer + ic2 LoRA")
    ...
    tmp_dir = OUT_ROOT / "_runner" / f"s{args.seed}_c{args.chunk}"
