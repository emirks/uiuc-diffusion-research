...
def main() -> None:
    ...
    with TeeLogger(run_dir / "run.log"):
        ...
        for idx, sample in enumerate(cfg["samples"]):
            ...
            for v in variants:
                name = v["name"]
                mode = v.get("mode", "global")
                gs   = float(v.get("guidance_scale", 1.0))
                gpt  = None
                if mode == "localized":
                    anchor_cfg = float(v["anchor_cfg"]); free_cfg = float(v["free_cfg"])
                    gpt = build_guidance_per_token(cmask_packed, anchor_cfg, free_cfg)
                    desc = f"localized anchor_cfg={anchor_cfg} free_cfg={free_cfg}"
                else:
                    desc = f"global cfg={gs}"
                log.info("  regen variant '%s' (%s) …", name, desc)
                t_v = time.perf_counter()
                z0_regen = inverter.regenerate(
                    z1, regen_steps, stage1_scheduler, gs, guidance_per_token=gpt, diag=None)
                ...
                log.info("    [%s] full PSNR=%.2f  FREE-MID PSNR=%.2f  (ssim_fm=%.3f lpips_fm=%.3f)  %.1fs",
                         name, m["full"]["psnr"], m["free_mid"]["psnr"], m["free_mid"]["ssim"],
                         m["free_mid"]["lpips"], time.perf_counter() - t_v)
                log.info("    per-latent-frame PSNR: %s", m["per_latent_frame_psnr"])
                sample_variants.append({"name": name, "mode": mode, "desc": desc, **m})
