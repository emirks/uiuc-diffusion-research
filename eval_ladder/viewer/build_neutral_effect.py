...
def arm_rows(a: dict) -> dict[str, dict[int, dict]]:
    ...
    for line in src.read_text().splitlines():
        ...
        if kind == "manifest":
            ...
            continue
        if a.get("rows_arm") and row.get("arm") != a["rows_arm"]:   # grid v3: stamped registries carry base twins too
            continue
        for s in SEEDS:
...
def external_gen(a: dict, r: dict, per_seed: dict, m: dict | None, ceil: dict,
    ...
    e = {
        ...
        "ceil": ceil.get(r["gt_pool_class"]), "tier": a["id"], "grid": a.get("grid", "v2"),
...
# ------------------------------------------------------------------------------- GRID v3 (2026-09-07)
#: The four paper arms x neutral/effect x the two grid-v3 families, generated by scripts/grid_v3/launch_gen.py
#: into contract-v2 subentries. `grid` tags every gen so the page can filter by grid (v2 = the 152-row grid the
#: runs answer; v3-hf = Higgsfield + reserve rows at 121 f; v3-ed = the EffectData tier at native 81 f).
#: `makes_cards` lets these entries CREATE cards for rows no run answers (the 245 new rows) — the runs still own
#: the paired Δ; the new cards simply have no run column. Scores: the pass-shaped eval entry
...
GRID_V3_TABLE = [  # canonical arm, gen dir, category, arm label, kind, subentry KK for (neutral hf, neutral ed, effect hf, effect ed)
...
def _grid_v3_entries() -> tuple[list[dict], list[tuple]]:
    ext, cat = [], []
    evals = sorted((REPO_ROOT / "store/evals").glob("028_grid_v3_paper_arms__dai__*"))
    for arm, gen_dir, category, arm_label, kind, kks in GRID_V3_TABLE:
        for i, (tier, fam) in enumerate([("neutral", "hf"), ("neutral", "ed"), ("effect", "hf"), ("effect", "ed")]):
            ha = f"{arm}_{tier}_v3" + ("ed81" if fam == "ed" else "")
            sub = REPO_ROOT / f"store/gens/{gen_dir}/{kks[i]:02d}_{tier}_v3{'ed81' if fam == 'ed' else ''}__dai"
            rows_file = sub / "grid.jsonl" if (sub / "grid.jsonl").exists() else LADDER / f"registry_{ha}.jsonl"
            scores = (evals[-1] / ha) if evals else (REPO_ROOT / "store/evals/028_grid_v3_paper_arms__dai__pending" / ha)
            grid = "v3-ed" if fam == "ed" else "v3-hf"
            ext.append({
                "id": ha, "score_id": "gridv3_v4", "kind": kind, "frames": 81 if fam == "ed" else 121,
                "no_twin": (kind == "baseline") or tier == "effect", "grid": grid, "makes_cards": True,
                "join_swap": (f"__{ha}__", "__ic_gen__"),
                # the grid's neutral tier is ONE prompt family shared by every arm (the kept rows are the
                ...
                "rows": ("registry", rows_file), "scores": scores,
                "prompt_kind": f"GRID v3 {tier.upper()} tier (prompts/{'01' if tier == 'neutral' else '01'}{'0' if (tier, fam) == ('neutral', 'hf') else '2' if (tier, fam) == ('neutral', 'ed') else '1' if fam == 'hf' else '3'}): "
                               + ("the same {S1}. sksz. [{S2}.] neutral grammar" if tier == "neutral" else "neutral + the reference's effect clause")
                               + ("; base arm: token stripped, prompt + endpoint conditioning, no reference" if kind == "baseline" else ""),
                "doc": "misc/2026-09-07_eval_grid_v2/PROPOSAL.md",
            })
            cat.append((ha, category, arm, arm_label, tier, f"{tier} · v3 {'ED81' if fam == 'ed' else 'HF'}"))
    return ext, cat


_GRID_V3_EXTERNAL, GRID_V3_CATALOG = _grid_v3_entries()
EXTERNAL.extend(_GRID_V3_EXTERNAL)
...
def build() -> dict:
    ...
    # grid v3: the v3 registry (a superset carrying the 139 kept rows verbatim + base twins) is a second extra
    v3 = LADDER / "registry_v3.jsonl"
    if v3.exists():
        run_eval.EXTRA_REGISTRY = ([run_eval.EXTRA_REGISTRY] if run_eval.EXTRA_REGISTRY else []) + [v3]
    rows = list({r["item_id"]: r for r in run_eval.load_registry()}.values())   # dedup: the kept rows appear in both
    ...
    def gen_entry(r: dict) -> dict | None:
        ...
        e = {
            ...
            "mismatched_ref": bool(r.get("mismatched_reference")),
            "ceil": ceil.get(r["gt_pool_class"]), "tier": tier_of(r), "grid": "v2",
    ...
    ext = attach_external(cards, registry, ceil, new_card)
    for card in cards.values():                   # grid membership of every card (v2 / v3-hf / v3-ed)
        card["grids"] = sorted({g.get("grid", "v2") for s in card["slots"].values() for g in s})
    ...
    makers = [a["id"] for a in EXTERNAL if a.get("makes_cards")]
    matrix = collections.Counter((g["novelty"], g["content"]) for g in treatments)
    for c in ordered:                              # cells only grid-v3 rows populate still count (once per card)
        for cell in {(g["novelty"], g["content"]) for t in makers for g in c["slots"].get(t, [])}:
            if not any((g["novelty"], g["content"]) == cell for t in run_tiers for g in c["slots"][t]):
                matrix[cell] += 1
    ...
    return {
        "meta": {
            ...
            "grids": [{"id": gid, "label": GRID_V3_LABEL.get(gid, gid),
                       "cards": sum(1 for c in ordered if gid in c.get("grids", []))}
                      for gid in ["v2", "v3-hf", "v3-ed"] if any(gid in c.get("grids", []) for c in ordered)],
