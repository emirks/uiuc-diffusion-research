# Ladder run record — v2.0.0-R1

**Status: VALID-WITH-AMENDMENTS** · design `v2.0.0` · run `R1` · 2026-07-23 · UIUC Campus Cluster
...
|---|---|
| run id | `v2.0.0-R1` |
| date | 2026-07-22 evening → 2026-07-23 midday |
| design version | `2.0.0` (`VERSION`) |
| `SPEC.md` sha256 | `cc341d192392f76d…` |

## 2. Instrument
| | |
|---|---|
| version | **transition-eval `4.0.0` — CERTIFIED (overall PASS)** |
| commit | `258a990` (worktree `.claude/worktrees/eval-v4-cert`) |
| appearance kernel | `m1a_S3` |
| distance matrices | `outputs/eval/certification/4.0.0-draft.1/analysis/distance_matrices.npz` |

**Disclosure on that path.** v4.0.0 is a *regrade* of the draft.1 run's artifacts, not a re-run:
only a grading clause changed (bar 8 rebuild-parity), no measurement or grader code. The matrices
under the `4.0.0-draft.1` directory are therefore the certified v4.0.0 measurements. This run did
**not** ride an uncertified instrument.

## 3. Arms
12 trained models — 11 c2v specialists (rank32/α32 attn, lr 1e-4, 2000 steps) + 1 IC-LoRA
generalist `ic_gen` (rank32/α32 attn+FFN, lr 2e-4, 5000 steps), plus `base` (no adapter) and
`text_floor` (prompt only). Configs: `train/configs/`, emitted by `train/make_configs.py`.
Checkpoints pinned in `arms.yaml` before any score existed.

## 4. Frozen inputs
| input | sha256 (first 16) |
|---|---|
| `registry.jsonl` (444 rows) | `25802eb66c6a2813` |
| `arms.yaml` | `52719cc0ed0763bf` |
| `davis.yaml` | `4985a4bf5adb9e77` |
| `split_v1.2.json` | `22f3f42bb215816c` |

## 5. Completion
**888/888 generations present** — verified row-by-row against the registry, 0 missing.
**11,695 unique scored (generation × pool-reference) rows** + 2,211 recipient-pool rows + 420
dominance rows. 87.9 GPU-h over 339 Slurm jobs.
*Disclosed:* 798 duplicate scored rows exist on disk from overlapping incremental passes; both
reporters dedup on the eval id (§8.4). 8 orphan videos from a pre-`hike` registry build are not in
`registry.jsonl` and were never scored.

## 6. Per-claim-cell verdicts

| claim cell | bar | measured | verdict |
|---|---|---|---|
| `SP-same` | Δpp > 0, ≥80 % donors | **99.7 % of ceiling, +40.0 pp, 11/11** | **PASS** |
| `SP-cross` | Δpp > 0, ≥80 % donors | **(94.9 %), +39.2 pp, 11/11** | **PASS** |
| `G-unseen-cross` | donor-pool Δpp | −27.2 pp, 0/13 | **INVALID** (demo-copy confound → Amendment-1) |
| `G-unseen-cross` | ΔTI > 0, ≥80 % donors *(Amendment-1)* | **+3.9 pp, 9/13** | **PASS** |
| `G-zs-cross` | donor-pool Δpp | −30.3 pp, 0/10 | **INVALID** (same) |
| `G-zs-cross` | ΔTI > 0, ≥80 % donors *(Amendment-1)* | −8.3 pp, 1/10 | **FAIL** |

Support and descriptive cells: `SP-fit` 100.4 % (anchor, ≈ `SP-same` → no train/test endpoint gap);
`text_floor` 67.4 % (leak-proof floor); `SP-foreign` (63.1 %), +18.9 pp, **8/11 — weak**, below the
80 % sign-test bar. All seven reference-bearing `G-*` support cells are **INVALID** for the same
confound and are retained with their numbers.

## 7. Amendments and deviations

**Amendment-1 — `pre-registered` (locked before any corrective number existed).** The donor-pool
margin is invalid for reference-bearing cells because the base twin can win by reproducing the
in-context demo. Replaced by the transfer index `TI = min(T, C)` (SPEC §11). Both resulting
verdicts — one PASS, one FAIL — stand, which is the point of having locked it first.

**Amendment-2 — `pre-registered`.** The planned step-4500-vs-5000 inline eyeball was replaced by a
scored checkpoint diagnostic on claim-bearing eval items, with the bar (UNDERSHOT if Δ ≥ +2.0 pp
AND ≥ 2/3 of items improve) declared before generating. *Status: running at time of writing;
diagnostic only, never a claim.*

**Deviation — owner call, `outcome-aware` in effect.** The DAVIS roster was expanded mid-run from a
16-item gate to 68 items. This **changed a conclusion**: at n=5 `SP-foreign` read 4/5 donors
positive; at full n=22 it is 8/11 — below bar. Disclosed because the smaller sample would have been
reported as a clean positive.

## 8. Kill rules
**None fired.** K0 satisfied by every job; K1 passed at every model's step 500; K2 and K3 never
approached. The new-variable gate passed at **step 250**, 4× earlier than its abort rule.

## 9. Artifacts and reading
| | |
|---|---|
| generations | `outputs/videos/ladder2/` |
| scores | `outputs/eval/ladder2/`, `outputs/eval/ladder2_recp/` |
| dominance | `dominance_passA.jsonl`, `dominance_passB.json` |
| viewer | `outputs/reports/ladder2/index.html` (build: `viewer/build.py`) |
| decision log | `$LAB/misc/ladder2_redesign/DOSSIER.md` |

> A specialist reaches its class ceiling on unseen endpoints of its own class and holds ~95 %
> transferring to another class's endpoints — ~+40 pp over its base twin on byte-identical inputs,
> 11/11 donor classes. The generalist's story only becomes readable after Amendment-1: in-context
> transfer **works inside the trained transition vocabulary** (+3.9 pp, 9/13 donors) and **fails on
> genuinely novel transitions** at this budget (−8.3 pp, 1/10). The mechanism behind the confound is
> itself the run's most interesting result: without the adapter the model treats an in-context clip
> as *content to continue*; the IC-LoRA converts it into *an instruction to imitate*.
...
**Mode:** `/advised` — operator (Opus) executes and verifies; `fable-advisor` owns every judgment call.
**Single source of truth:** `eval_ladder/registry.jsonl`. Dossier: `$LAB/misc/ladder2_redesign/DOSSIER.md`. Code: branch `ladder2`.
...
All numbers below are regenerated by `eval_ladder/report_full.py` (deduped). Base rows carry their own cell labels (`base:<cell>`) because a single base twin can serve more than one treatment cell.
...
1. **Convergence diagnostic** (ckpt-4500 vs ckpt-5000 on G-unseen-same + G-zs-cross) — ~~running~~ **RESOLVED: CONVERGED** — see §14. Pre-declared bar: **UNDERSHOT** if Δ ≥ +2.0 pp **and** ≥ 2/3 of items improve → authorises a labelled follow-up run and flags ic_gen-vs-specialist as budget-confounded; **CONVERGED** if |Δ| < 2.0 pp or non-systematic. This decides whether §9's zero-shot loss is a real limit or a budget artifact.
...
## 14. Open-item closure — convergence diagnostic (appended 2026-07-23 14:35)

Closes open item 1. Generation from ckpt-4500: jobs 9644216/9644217 (66 videos, arm dir
`ic_gen__ck4500`, both seeds). Scoring: job 9646200, 322 (generation × pool-reference) rows under
the identical v4 kernel, pool rule and ceilings as the main run. Verdict computed by
`convergence_diagnostic.py --mode report` — the bar was fixed in that file's docstring before any
ic_gen score existed and was not edited since.

| cell | n | ckpt-4500 | ckpt-5000 (pinned) | Δpp (5000−4500) | items improved |
|---|---|---|---|---|---|
| G-unseen-same | 13 | 86.5 % | 88.7 % | +2.2 | 7/13 (54 %) |
| G-zs-cross | 20 | 73.0 % | 72.7 % | −0.3 | 9/20 (45 %) |

**Verdict: CONVERGED.** Applying the pre-declared conjunction:

- `G-unseen-same` crosses the magnitude arm (+2.2 pp ≥ +2.0) but **fails the systematicity arm**
  (7/13 = 54 % < 2/3): the mean is carried by a few items, not by a coherent ascent. Under the
  bar as declared, that is noise-compatible — not UNDERSHOT, and not AMBIGUOUS either (that band
  was Δ ∈ [1.0, 2.0) *with* a systematic sign).
- `G-zs-cross` — **the cell that carries the negative claim** — is flat to the tenth of a point
  (−0.3 pp, 9/20 improved, a coin flip). The final 500 training steps bought it nothing.

**Consequences, as pre-registered:** step-5000 stands as the pinned checkpoint; no follow-up run
is authorised; ic_gen-vs-specialist comparisons are **not** budget-confounded; and §9's zero-shot
FAIL (−8.3 pp, 1/10 donors) is a real property of this budget, not an artifact of stopping while
the model was still climbing.

**Scope, stated honestly:** the diagnostic compares 4500 against 5000 and nothing else. It removes
the specific confound it was declared for — "5000 was still on an ascent" — and cannot speak to
what a 10k-step or larger-rank run would do. That question, if ever asked, is a new pre-registered
run, not a reinterpretation of this one.

Diagnostic status unchanged: **never enters the claim tables** (§9 is complete without it).
...
python eval_ladder/report_full.py

# copy confound (§8) and transfer index (§9)
python eval_ladder/dominance.py --mode reportA
python eval_ladder/dominance.py --mode reportB
```
