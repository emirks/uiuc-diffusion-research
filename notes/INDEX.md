...
| [`dataset/procedural_operators.md`](dataset/procedural_operators.md) | Dataset | Procedural transition operator engine — gl-transitions bank (125 shaders / 189 uniforms / licences), the endpoint-identity gate, headless moderngl+EGL recipe & the llvmpipe-vs-NVIDIA trap, ffmpeg xfade + `xfade-easing` fallbacks (measured 128× slower), diversity multipliers, endpoint-corpus sources, the task-diversity phase-transition bibliography (Raventós et al.), and **§5b luma-matte family (exp_084): a static matte IS an arrival-time map; `step()` silently breaks the endpoint identity and the aux sampler is y-flipped; blind 2×2 shows the compositor gates everything (better maps through `step()` buy nothing, p=1.00), the 3 aperiodic shipped maps are rescued by a feather (6/8→1/8 BAD) and the 4 geometric ones are not (8/8→8/8); no CC0 commercial matte pack exists** |
| [`dataset/humanvid_real.md`](dataset/humanvid_real.md) | Dataset | HumanVid's REAL half = 19,262 **Pexels URLs** (Drive manifest + per-frame cameras), not media. **BLOCKED: Pexels ToS bar scraping/bulk copying "for machine learning purposes" and bar API collection to "train, fine-tune, evaluate, or develop ML/AI models or datasets" — the repo's Apache-2.0/CC-BY cannot relicense it.** Full zero-download characterisation (90.2 h, 167 GB projected, fps/orientation/duration), fitness vs our 480×640·121f·24fps contract, and the streaming viewer |
...
| [`eval_feature_store.md`](eval_feature_store.md) | Eval | **Feature store (since 2026-09-17): every metric feature lives next to its video** (`features/<item>/<ns>.npz`+`.json`; store contract clause 10; registry `store/FEATURES.md`); `scripts/store_features.py coverage|fsck|sha256sums|migrate|extract --population …`; harness 4.0.1-draft.1 `--store-root`, lenses `--store`; no `--cache-dir` anywhere; migration facts + gotchas |
...
**`humanvid_real.md`** — *Investigated 2026-07-27; outcome is a clear negative.* Where HumanVid's real/Internet half actually lives (Google Drive URL lists + `camera_tram.zip`, **not** HuggingFace), and why we cannot use it: the clips are third-party **Pexels** footage whose ToS prohibit automated collection "for machine learning purposes" and bulk copying, with the API terms separately barring collection to train/fine-tune/**evaluate** ML models **or datasets**. Includes the full corpus characterisation done **without downloading anything** (URLs encode W/H/fps; camera files have one line per frame — validated 10/10 against `ffprobe`), the fitness verdict against our endpoint contract, and the CDN-streaming viewer that lets the owner inspect samples without copying them.
...
**`eval_feature_store.md`** — Where every metric feature lives and how scorers reach it: the colocated `features/` rule (gens, corpus, endpoint clips, controls), the frozen namespace registry, the CLI (`coverage|fsck|sha256sums|migrate|extract`, `--population`), the harness/lens/certification entrypoints on the store model, and the migration facts (legacy hash caches, hard-link migration of population gridv3, bit-identical reproduction of evals/028, the recovered `reference_v4.npz` 459fd9a7).
