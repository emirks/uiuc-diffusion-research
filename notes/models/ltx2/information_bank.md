...
Deeper vendored-path notes: [`conditioning_mechanism.md`](conditioning_mechanism.md).
...
**Practical takeaway:** Diffusers VC is **in-grid** at **latent** indices; vendored C2V uses **appended** clips with **pixel** `frame_idx`. Do not assume the same integer means the same thing across stacks.
...
- [`README.md`](README.md) — index of LTX-2 notes  
- [`conditioning_mechanism.md`](conditioning_mechanism.md) — vendored patchifier, masks, RoPE, exp_014/015  
- `src/LTX-2/packages/ltx-pipelines/README.md` — upstream pipeline CLI docs  
