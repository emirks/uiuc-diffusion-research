...
## Addendum 2026-09-12 — what the on-line R3 clips are

Of the 25 scene-change R3 clips on the endpoint line, 23 are cut-like (M < 0.15) and none is dissolve-like (M > 0.30). Per-frame τ profiles: the start scene is held for roughly the first third of the interior, the crossing happens at 0.34 of the interior (median), and the end scene is held for the rest. The off-line R3 scene-change clips share this shape at DR 0.19–0.33. So the base model's captions-only behaviour on scene-change endpoints is to reach the end scene early and stay there; the on-line subset is where that crossing is a clean cut between two static holds. Details in TABLES.md §H. An earlier wording in §H ("jump in the last frames") misread τ_med and was corrected.
